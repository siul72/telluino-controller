    ...
    VpProfilePtrType pSeqData)
{
 ...
 || (defined (VP_CC_890_SERIES) && defined (VP890_FXS_SUPPORT))
        ...
        if ((pCid != VP_NULL) && (pCid->status & VP_CID_WAIT_ON_ACTIVE)) {
            VP_CID(VpLineCtxType, pLineCtx, ("CID Status 0x%08X", pCid->status));
    ...
    uint16 updateInterval)  /**< INPUT: Specifies the sequencer update rate (based on tickrate) */
{
    ...
    VP_SEQUENCER(None, VP_NULL,
                 ("Num Steps %d From sweepInteval 0x%08X and updateInterval 0x%04X",
    ...
    uint16 byteMask)    /**< INPUT: Second multiplication value in 1.15-bit format */
{
    ...
    VP_SEQUENCER(None, VP_NULL,
             ("Result of 0x%04X times byteMask 0x%04X (1.15 format) with error (0x%04X) is 0x%08X",
    ...
    VpLineCtxType *pLineCtx)    /**< Line that has an active CID sequence */
{
    ...
    if (index <= pCidStruct->pCliProfile[startOfCliData - 2]) {
        ...
        switch (pCidStruct->pCliProfile[startOfCliData + index]) {
            ...
            case VP_CLI_SILENCE_MASKHOOK:
                ...
                pCidStruct->cliDebounceTime = (uint8)MS_TO_TICKRATE(tempDebounceTime, tickRate);
                index+=2;
                break;
    ...
    if (index > pCidStruct->pCliProfile[startOfCliData - 2]) {
        ...
        pCidStruct->cliIndex = (uint8)index;
    ...
    VpProfilePtrType pMeterProfile)
{
    ...
    switch (deviceType) {
        ...
        case VP_DEV_880_SERIES:
            ...
            meterProfEntry =
                (uint8)(((Vp880DeviceObjectType *)pDevObj)->profEntry.meterProfEntry);
            break;
#endif
        default:
    ...
    uint16 numMeters)
{
    ...
    if (timeRemaining == 0) {
        ...
        tickAdjust = (uint8)TICKS_TO_MS(1, tickRate);
        if (tickAdjust % 5 > 2) {
    ...
    if (timeRemaining == 0) {
        ...
        tickAdjust = (uint8)TICKS_TO_MS(2, tickRate);
        if (tickAdjust % 5 > 2) {
            tickAdjust = (tickAdjust / 5) + 1;
        } else {
            tickAdjust = (tickAdjust / 5);
...
#endif /* (defined (VP_CC_880_SERIES) && defined (VP880_FXS_SUPPORT)) */
