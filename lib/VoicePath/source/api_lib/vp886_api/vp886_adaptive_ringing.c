    ...
    void* arg)
{
   ...
   /* Compute the integral of the power dissipated by the SLIC. */
...

    {
        ...
        int16 fScale = (int16)VpRoundedDivide(620, samples);
        ...
        /* Prevent compiler warnings when ADP_RING debug is not included */
        (void)slicPwrDbg;
        (void)fusePwrDbg;
        (void)batPwrDbg;
        ...
        if (verboseDebug) {
            VP_ADP_RING(VpLineCtxType, pLineCtx, ("L = %4d mW, ringV = %3d V, threshold = %4d mW, dCntLow = %d, dCntHigh = %d, ts = %5d ms",
                loadPwrDbg, ((pLineObj->ringAmplitudeCal * pRPACD->ringGain) >> 15) * (int32)473 / 100000, pRPA->rspt, pRPA->debCounterLow, pRPA->debCounterHigh, tStamp / 2));
        } else {
            VP_ADP_RING(VpLineCtxType, pLineCtx, ("L = %4d mW", loadPwrDbg));
        }
    }
#endif
    ...
    int16 samples)
{
    ...
    if (!((pDevObj->options.adaptiveRinging.mode == VP_ADAPT_RING_SHARED_TRACKER) ||
        ...
        (pDevObj->options.adaptiveRinging.mode == VP_ADAPT_RING_FIXED_TRACKER) ||
        (pDevObj->options.adaptiveRinging.mode == VP_ADAPT_RING_FULL_TRACKER) )) {
    ...
    RingPowerAdaptChannelData* pRPACD)
{
    ...
    switch (pDevObj->options.adaptiveRinging.mode) {
        ...
        case VP_ADAPT_RING_FIXED_TRACKER:
            /* Update battery voltage */
            battery_V = (pLineObj->refRingingBatV * pRPACD->ringGain + FULL_RINGING_GAIN / 2) / FULL_RINGING_GAIN;
            ...
            VpMemCpy(pLineObj->registers.swParam, swParam, VP886_R_SWPARAM_LEN);
        ...
        case VP_ADAPT_RING_SHARED_BB_ABS:
            ...
            pDevObj->absRingingPeak[pLineObj->channelId] = (uint8)((ABS(amplitude) + ABS(dcBias) + 999) / 1000);

            /* Update the battery voltage (apply cal) */
            Vp886ManageABSRingingBatt(pDevCtx, TRUE, TRUE);
            break;

        case VP_ADAPT_RING_FULL_TRACKER:
            /* Do not try to change battery */
            break;
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    /**************************************************************
     *
     * How the buffer indices are used, incremented, wrapped, etc.
     *
 ...
 *  How do we coordinate this with ring enter/exit and ringtrip
 *  How/when to the amplitude corrections get applied?
 *
 *
 * Inputs:
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    if ((pRPACD->ringGain != FULL_RINGING_GAIN) &&
        ...
        /* 
         * DO NOT restore the ringing floor voltage.
         */
