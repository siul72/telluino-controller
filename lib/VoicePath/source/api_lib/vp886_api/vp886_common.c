    ...
    VpProfilePtrType *ppProfileArg)
{
    ...
    /* If profile argument is an index, get a pointer to the profile in the table. */
        ...
        /* ensure the profType is valid for the table */
        if (profType >= VP886_NUM_PROFILE_TYPES) {
            VP_ERROR(VpDevCtxType, pDevCtx, ("Profile type %s not included in profile table", VpGetString_VpProfileType(profType)));
            return VP_STATUS_ERR_PROFILE;
        } else if (profIndex >= ProfileTableDims[profType].numProfiles) {
    ...
    switch (profType) {
        ...
        case VP_PROFILE_TONECAD:
            ...
            break;
        case VP_PROFILE_DEVICE:
            /* This will be checked later, in order to allow cases where we
               change the device from tracker to ABS or vice versa. */
            break;
        case VP_PROFILE_DC:
        case VP_PROFILE_RING:
        case VP_PROFILE_CAL:
    ...
    VpStatusType *pStatus)
{
    ...
    if (pDevCtx != VP_NULL) {
        ...
        *pStatus = VP_STATUS_FAILURE;
        VP_ERROR(None, VP_NULL, ("Vp886ReadyStatus - pDevCtx NULL error"));
        return FALSE;
     }

    /* Check for "device not ready" conditions. */
    busyFlags = pDevObj->busyFlags;
    ...
    if (pDevObj->spiError.block) {
        *pStatus = VP_STATUS_ERR_SPI;
        if (pLineCtx != VP_NULL) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("SPI error threshold exceeded"));
        } else {
            VP_ERROR(VpDevCtxType, pDevCtx, ("SPI error threshold exceeded"));
        }
        return FALSE;
    }
    if (pDevObj->options.accessCtrl == VP_ACCESS_CTRL_BLOCK_DEVICE) {
        *pStatus = VP_STATUS_ACCESS_BLOCKED;
        return FALSE;
    }
...
#define CSD_NIBBLES 4
    ...
    */
    for (n = 1; n <= CSD_NIBBLES; n++) {
        ...
        if (sum < fixed) {
            t[n].sign = 0;
            error = (uint16)(fixed - sum);
        } else {
            t[n].sign = 1;
            error = (uint16)(sum - fixed);
        }
    ...
    char* funcName)
{
    ...
    /* If nested critical section detected, print the list of functions that are
       currently in critical sections, including the current one */
    if (pDevObj->criticalDepth > 1 && (pDevObj->debugSelectMask & VP_DBG_WARNING)) {
        VP_WARNING(VpDevCtxType, pDevCtx, ("Nested critical section"));
        for (n = 1; n < pDevObj->criticalDepth && n <= VP886_CRITSEC_STRING_NUM; n++) {
            VpSysDebugPrintf("\tDepth %d: %s\n",
                n, pDevObj->critSecFuncs[n-1]);
        }
        if (n < pDevObj->criticalDepth) {
            VpSysDebugPrintf("\t...\n");
        }
        VpSysDebugPrintf("\tDepth %d (this): %s\n",
            pDevObj->criticalDepth, funcName == NULL ? "(null)" : funcName);
    }

    /* Add the current function name to the list, if there is room */
    if (pDevObj->criticalDepth <= VP886_CRITSEC_STRING_NUM) {
        n = pDevObj->criticalDepth - 1;
        if (funcName == NULL) {
            pDevObj->critSecFuncs[n][0] = '(';
            pDevObj->critSecFuncs[n][1] = 'n';
            pDevObj->critSecFuncs[n][2] = 'u';
            pDevObj->critSecFuncs[n][3] = 'l';
            pDevObj->critSecFuncs[n][4] = 'l';
            pDevObj->critSecFuncs[n][5] = ')';
            pDevObj->critSecFuncs[n][6] = 0;
        } else {
            i = 0;
            while (i < VP886_CRITSEC_STRING_LENGTH && funcName[i] != 0) {
                pDevObj->critSecFuncs[n][i] = funcName[i];
                i++;
            }
            pDevObj->critSecFuncs[n][i] = 0;
        }
    }
}
#endif /* defined(VP_DEBUG) && (VP_CC_DEBUG_SELECT & VP_DBG_WARNING) */
...
/** Vp886SetSwTimingParams()
  ...
  restore normal timing parameters when exiting free-run or battery backup mode.
*/
void
Vp886SetSwTimingParams(
    VpDevCtxType *pDevCtx)
{
    ...
    if (pDevObj->stateInt & VP886_FREE_RUN) {
        VpMemCpy(intSwParam, pDevObj->swTimingParamsFR, VP886_R_SWTIMING_LEN);
        swCtrl = pDevObj->devProfileData.swCtrlFreeRun;
    } else if (pDevObj->stateInt & VP886_BATTERY_BACKUP) {
        VpMemCpy(intSwParam, pDevObj->swTimingParamsBattBackup, VP886_R_SWTIMING_LEN);
        swCtrl = pDevObj->devProfileData.swCtrlBattBackup;
    } else { /* Normal mode */
        ...
        swCtrl = pDevObj->devProfileData.swCtrlNormal;
    }

    if (pDevObj->initDeviceState == VP886_INIT_DEVICE_SWITCHER_PREP &&
        pDevObj->softStartDuration != 0)
    {
        /* Soft startup - start off with lower low power timings to
           reduce peak currents during switcher startup. */
        intSwParam[0] = pDevObj->swTimingParamsSoftStart[0];
        intSwParam[1] = pDevObj->swTimingParamsSoftStart[1];
    }

    /* Update only the SWFS field of swCtrl.  If swCtrl == 0xFF, it means it
       was not included in the device profile (old version) so don't use it. */
    if (swCtrl != 0xFF) {
        /* Also check that the new SWFS would be different than the current, to
           avoid wasteful MPI traffic */
        if ((swCtrl & VP886_R_SWCTRL_FREQ) != (pDevObj->registers.swCtrl[0] & VP886_R_SWCTRL_FREQ)) {
            pDevObj->registers.swCtrl[0] &= ~VP886_R_SWCTRL_FREQ;
            pDevObj->registers.swCtrl[0] |= (swCtrl & VP886_R_SWCTRL_FREQ);
            VpSlacRegWrite(pDevCtx, NULL, VP886_R_SWCTRL_WRT, VP886_R_SWCTRL_LEN,
                pDevObj->registers.swCtrl);
        }
    }

    /* Update timing params only if something has changed */
    for (i = 0; i < VP886_R_SWTIMING_LEN; i++) {
        if (intSwParam[i] != pDevObj->registers.swTiming[i]) {
            VpMemCpy(pDevObj->registers.swTiming, intSwParam, VP886_R_SWTIMING_LEN);
            VpSlacRegWrite(pDevCtx, NULL, VP886_R_SWTIMING_WRT, VP886_R_SWTIMING_LEN,
                pDevObj->registers.swTiming);
            break;
        }
    }
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    /* If the ICR settings have been changed for VpGetLoopCond(), restore the
       cached settings first */
