    ...
    VpDevCtxType *pDevCtx)
{
    ...
    if (pDevCtx == VP_NULL && pLineCtx == VP_NULL) {
        ...
        if (pLineCtx != VP_NULL) {
            ...
            VpSysDebugPrintf("\tpLineCtx->pDevCtx = %p\n", (void *)pLineCtx->pDevCtx);
            VpSysDebugPrintf("\tpLineCtx->pLineObj = %p\n", (void *)pLineCtx->pLineObj);
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    switch (pDevCtx->deviceType) {
        ...
        case VP_DEV_MELT792_SERIES:
            return VpDebugEnabled_VpMelt792DeviceObjectType(msgType, msgTypeStr, pDevCtx->pDevObj);
#endif
#ifdef VP_CC_890_SERIES
...
#ifdef VP_CC_MELT792_SERIES
        case VP_DEV_MELT792_SERIES:
            return VpDebugEnabled_VpMelt792LineObjectType(msgType, msgTypeStr, pLineCtx->pLineObj);
#endif
#ifdef VP_CC_890_SERIES
        case VP_DEV_890_SERIES:
    ...
    VpOptionIdType optionId)
{
    ...
    VP_ASSERT(VP_NUM_OPTION_IDS == 0x2A);
    ...
    VpEventType *pEvent)
{
    ...
    static const VpEventInfo eventInfoTable[] = {
        ...
        { VP_EVCAT_PROCESS,   VP_DEV_EVID_SEAL_CUR_OFF,        "VP_DEV_EVID_SEAL_CUR_OFF",  TRUE, FALSE },        
            ...
            case VP_LINE_EVID_QUERY_CMP: {
                ...
                switch (pEvent->eventData) {
                    ...
                    case VP_QUERY_ID_DEV_TRAFFIC:
                        pOut += sprintf(pOut, "trafficBytes=%u", pQueryResults->trafficBytes);
                        break;
                    case VP_QUERY_ID_LINE_CAL_COEFF:
                        break;
                    case VP_QUERY_ID_TIMESTAMP:
                        pOut += sprintf(pOut, "timestamp=0x%08X", pQueryResults->timestamp);
                        break;
                    case VP_QUERY_ID_LINE_TOPOLOGY:
                        ...
                        break;
                    case VP_QUERY_ID_SEAL_CUR_STATUS:
                        pOut += sprintf(pOut, "sealCurStatus={");
                        pOut += sprintf(pOut, "running=%u ", pQueryResults->sealCurStatus.running);
                        pOut += sprintf(pOut, "currentLine=%u ", pQueryResults->sealCurStatus.currentLine);
                        break;
                    case VP_QUERY_ID_SW_DUTY_CYCLE:
                        pOut += sprintf(pOut, "onTime=%u ", pQueryResults->swDutyCycle.onTime);
                        pOut += sprintf(pOut, "cycleCount=%u ", pQueryResults->swDutyCycle.cycleCount);
                        pOut += sprintf(pOut, "avgDutyCycle=%u ", pQueryResults->swDutyCycle.avgDutyCycle);
                        pOut += sprintf(pOut, "stateChanged=%u ", pQueryResults->swDutyCycle.stateChanged);
                        break;
                    default:
                        /* Unknown Query ID. */
     ...
     defined(VP_CC_MELT792_SERIES))
        ...
        case VP_DEV_MELT792_SERIES:
            if (cmd == 0xFFFF) {
                ...
                sprintf(output, "%4.4X%s%s Offset %s (%u word%s at offset 0x%2.2X)", 
                    (int)cmd,
                    HBI_COMMAND_LINE_FILLER, 
                    (direct ? "Direct" : "Paged"),
                    (write ? "Write" : "Read"), 
                    dataLength, 
                    (dataLength > 1 ? "s" : ""),
...
#endif /* ((VP_CC_DEBUG_SELECT & VP_DBG_HAL) && (defined(VP_CC_VCP2_SERIES) || defined(VP_CC_MELT_SERIES) || 
    defined(VP_CC_VCP_SERIES) || defined(VP_CC_MELT792_SERIES) )) */
    ...
    VpCallerIdType *callerId)
{
    ...
    VpSysDebugPrintf("\n\npLineObj->callerId.status = 0x%08X", callerId->status);
    ...
    VpCslacTimerStruct *lineTimers)
{
 ...
 || (defined(VP_CC_890_SERIES) && defined(VP890_FXO_SUPPORT))
        ...
        char *timerNames[VP_LINE_TIMER_LAST] = {
            ...
            "VP_LINE_RING_EXIT_PROCESS", "VP_LINE_HOOK_FREEZE",
            "VP_LINE_DISCONNECT_EXIT", "VP_LINE_GND_START_TIMER",
            "VP_LINE_CAL_LINE_TIMER", "VP_LINE_PING_TIMER",
            "VP_LINE_TRACKER_DISABLE", "VP_LINE_INTERNAL_TESTTERM_TIMER",
    ...
    void *devProfileData)
{
    if (deviceType == VP_DEV_880_SERIES) {
        ...
        VpSysDebugPrintf("\n\rpDevObj->devProfileData.swCtrlNormal = 0x%02X",
            ((Vp886DeviceProfileType *)devProfileData)->swCtrlNormal);
        VpSysDebugPrintf("\n\rpDevObj->devProfileData.swCtrlFreeRun = 0x%02X",
            ((Vp886DeviceProfileType *)devProfileData)->swCtrlFreeRun);
        VpSysDebugPrintf("\n\rpDevObj->devProfileData.swCtrlBattBackup = 0x%02X",
            ((Vp886DeviceProfileType *)devProfileData)->swCtrlBattBackup);
        VpSysDebugPrintf("\n\rpDevObj->devProfileData.batSelGpio = %d",
            ((Vp886DeviceProfileType *)devProfileData)->batSelGpio);
        VpSysDebugPrintf("\n\rpDevObj->devProfileData.vblUseMedTiming = %s",
            ((Vp886DeviceProfileType *)devProfileData)->vblUseMedTiming ? "TRUE" : "FALSE");
#endif
    ...
    uint16 devTimer[VP_DEV_TIMER_LAST])
{
    ...
    char *timerNames[VP_DEV_TIMER_LAST] = {
        "VP_DEV_TIMER_TESTLINE", "VP_DEV_TIMER_ABSCAL",
        "VP_DEV_TIMER_LP_CHANGE", "VP_DEV_TIMER_ABV_CAL",
        "VP_DEV_TIMER_ENTER_RINGING", "VP_DEV_TIMER_EXIT_RINGING",
        "VP_DEV_TIMER_WB_MODE_CHANGE"
    };
    ...
    VpTimerQueueNodeType *pNodes)
{
    ...
    VpSysDebugPrintf("\n\n\rpDevObj->timerQueue (front = %d, timestamp = %u):", pInfo->index, pInfo->timestamp);
    for (idx = 0; idx < pInfo->numNodes; idx++) {
        VpSysDebugPrintf("\n\r\t%2d: active %d, next %2d, id %4d, ch %d, dur %8u, hndl %u",
