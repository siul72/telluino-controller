    ...
    VpDevCtxType *pDevCtx)
{
    ...
    VpSlacRegRead(pDevCtx, NULL, VP886_R_FUSE_REG_8_RD, VP886_R_FUSE_REG_8_LEN,
        pDevObj->registers.fuse8);
    ...
    bool initDevice)
{
    ...
    if (pDcProfile != VP_NULL) {
        ...
        if (VP886_IS_TRACKER(pDevObj)) {
            ...
            lpBatt = (uint8)(VpRoundedDivide(pLineObj->targetVoc + 4, 5) - 1);
        ...
        /* Get the mid battery compensation flag and value if ABS.  Changes will
           be handled by the apply cal functions. */
        if (pDcProfile[VP_PROFILE_VERSION] >= 3 && VP886_IS_ABS(pDevObj)) {
            if ((*pData) & 0x10) {
                pLineObj->midBatCompEnable = TRUE;
                pLineObj->midBatCompValue = ((*pData) & VP886_R_INDCAL_60VCLAMP);
            } else {
                pLineObj->midBatCompEnable = FALSE;
                pLineObj->midBatCompValue = 0;
            }
            pData++;
        } else {
            pLineObj->midBatCompEnable = FALSE;
        ...
        /* Get the 1V-step VOC target */
        if (pDcProfile[VP_PROFILE_VERSION] >= 3) {
            pLineObj->targetVoc = *pData++;
        }
    ...
    if (pRingProfile != VP_PTABLE_NULL) {
        ...
        if (VP886_IS_ABS(pDevObj)) {
            ...
            if ((amplitude_mV + bias_mV) > 154400) {
                VP_WARNING(VpLineCtxType, pLineCtx, ("Requested ringing levels exceed limits amplitude_mV %d bias_mV %d", amplitude_mV, bias_mV));
                pDevObj->absRingingPeak[channelId] = 154;
            } else {
    ...
    /* Add the  updated Loop Supervision Parameters and Switching Regulator
    ...
    if ((pDcProfile != VP_NULL) || (pRingProfile != VP_NULL)) {
        ...
        if (VP886_IS_TRACKER(pDevObj)) {
            ...
            pLineObj->refRingingBatV = ((swParamBuf[1] & VP886_R_SWPARAM_RINGING_V) * 5 + 5);
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    if (pDevCtx->deviceType == VP_DEV_886_SERIES) {
        ...
        VP_INFO(VpDevCtxType, pDevCtx, ("Converting Tracker device to ABS."));
    } else {
        /* Setting this bit changes an ABS PCN to tracker */
        fuse7 |= VP886_R_RCNPCN_PCN_ABS_TRACKER;
        VP_INFO(VpDevCtxType, pDevCtx, ("Converting ABS device to Tracker."));
    }
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    if (pDevObj->devProfileData.swCfg == VP886_DEV_PROFILE_SW_CONF_BB) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886InitBatSelGpio - BBABS and GPIO battery switch are not compatible."));
        return VP_STATUS_ERR_PROFILE;
    }

    if (pDevObj->ioCapability == VP886_IO_CAPABILITY_NONE) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886InitBatSelGpio - batSelGpio %d invalid, device does not support GPIOs",
    ...
    {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886InitBatSelGpio - batSelGpio %d invalid, device only supports IO21",
            pDevObj->devProfileData.batSelGpio));
        return VP_STATUS_ERR_PROFILE;
    }
    ...
    if ((io21Usage == VP886_DEV_PROFILE_IO2_USE_DIG_INT || io21Usage == VP886_DEV_PROFILE_IO2_USE_VMM) &&
        pDevObj->devProfileData.batSelGpio == VP886_DEV_PROFILE_BAT_SEL_GPIO_IO21)
    {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886InitBatSelGpio - IO21 profile conflict, io21Usage %d, batSelGpio %d",
    ...
    {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886InitBatSelGpio - IO22 profile conflict, io22Usage %d, batSelGpio %d",
    ...
    switch (pDevObj->devProfileData.batSelGpio) {
        case VP886_DEV_PROFILE_BAT_SEL_GPIO_NONE:
            VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886InitBatSelGpio - batSelGpio is NONE, should not be possible here\n"));
        ...
        case VP886_DEV_PROFILE_BAT_SEL_GPIO_IO22:
            ...
            break;
        default:
            VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886InitBatSelGpio - Unknown batSelGpio %d\n", pDevObj->devProfileData.batSelGpio));
            return VP_STATUS_ERR_PROFILE;
    }
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    */
    ...
    {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: Test 4 failed. Write buffer corrupted: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            writeBuffer[0], writeBuffer[1], writeBuffer[2], writeBuffer[3], writeBuffer[4], writeBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    if (readBuffer[0] != 0x94 || readBuffer[5] != 0x94) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: Test 4 failed. Read buffer overflow: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            readBuffer[0], readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4], readBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    if (readBuffer[1] != 0x00 || readBuffer[2] != 0xFF || readBuffer[3] != 0x00 || readBuffer[4] != 0xFF) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: Test 4 failed. Read value incorrect: 0x%02X 0x%02X 0x%02X 0x%02X",
            readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4]));
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: expected 0x00 0xFF 0x00 0xFF"));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    /*
        Test 5:
        Test large read length.
    */
    VpMemSet(readBuffer, 0x94, sizeof(readBuffer));

    /* Read the voice test data buffer.  It is 25 bytes long, and should read
       back as all 0x00 after a device reset. */
    VpMpiCmdWrapper(deviceId, VP886_EC_1, VP886_R_VBUFFER_RD, VP886_R_VBUFFER_LEN, &readBuffer[1]);

    /* Make sure there was no overflow into the surrounding data */
    if (readBuffer[0] != 0x94 || readBuffer[1+VP886_R_VBUFFER_LEN] != 0x94) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: Test 5 failed. Read buffer overflow: 0x%02X --- 0x%02X",
            readBuffer[0], readBuffer[1+VP886_R_VBUFFER_LEN]));
        status = VP_STATUS_ERR_SPI;
    }

    /* Make sure the data was read back correctly. */
    for (i = 1; i < 1+VP886_R_VBUFFER_LEN; i++) {
        if (readBuffer[i] != 0x00) {
            VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: Test 5 failed. Read value %d incorrect: 0x%02X",
                i, readBuffer[i]));
            VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: expected 0x00"));
            status = VP_STATUS_ERR_SPI;
        }
    }

    /* Perform another quick read to make sure the MPI bus is not out of sync.
       This is the register that was written in test 1, and should read back as
       0x65. */
    VpMpiCmdWrapper(deviceId, VP886_EC_GLOBAL, VP886_R_CLKSLOTS_RD, VP886_R_CLKSLOTS_LEN, &readBuffer[1]);
    if (readBuffer[1] != 0x65) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: Test 5 failed. Read value incorrect: 0x%02X, expected 0x65",
            readBuffer[1]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    */
    ...
    /* Make sure that both the first and last writes of the large buffer read
       back correctly.
       Some bits of the register read back as 0, so DA becomes 5A
       and 99BB5678 becomes 01BB0678 */
    ...
    */
    ...
    /* Perform another quick read to make sure the MPI bus is not out of sync.
       This is the register that was written in test 3, and should read back as
       0x5A. */
    VpMpiCmdWrapper(deviceId, VP886_EC_GLOBAL, VP886_R_CLKSLOTS_RD, VP886_R_CLKSLOTS_LEN, &readBuffer[1]);
    if (readBuffer[1] != 0x5A) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: Test 5 failed. Read value incorrect: 0x%02X, expected 0x5A",
            readBuffer[1]));
        status = VP_STATUS_ERR_SPI;
    }
