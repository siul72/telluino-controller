...
#include "melt792_api_int.h"
#endif

#if defined (VP_CC_886_SERIES) || defined (VP_CC_MELT792_SERIES)
    ...
    VpTimerQueueNodeType *pNodes)
{
    ...
    VP_INFO(VpDevCtxType, pDevCtx, ("VpUpdateTimersInt: old %u, new %u, elapsed %u", pInfo->timestamp, newTimestamp, elapsed));
    ...
    while (pTimer->active && elapsed > 0) {
        VP_INFO(VpDevCtxType, pDevCtx, ("VpUpdateTimersInt: timer duration: %u", pTimer->duration));
        ...
        VP_INFO(VpDevCtxType, pDevCtx, ("VpUpdateTimersInt: new duration %u, overrun: %u", pTimer->duration, pTimer->overrun));
    ...
    uint32 handle)
{
    ...
    if (VpFindTimerInt(pInfo, pNodes, timerId, channelId, handle) != VP_END_OF_TIMER_QUEUE) {
        VP_TIMER(VpDevCtxType, pDevCtx, ("VpInsertTimer - Timer already exists (id %d, ch %d, hndl %u)", timerId, channelId, handle));
        return FALSE;
    }
    ...
    uint32 handle)
{
    switch (pDevCtx->deviceType) {
        ...
        case VP_DEV_MELT792_SERIES:
            Melt792TimerHandler(pDevCtx, timerId, overrun, channelId, handle);
            break;
...
#ifdef VP_CC_MELT792_SERIES
        case VP_DEV_MELT792_SERIES:
            timestamp = Melt792GetTimestamp32(pDevCtx);
            break;
#endif /* VP_CC_MELT792_SERIES */
        default:
    ...
    VpTimerQueueNodeType *pNodes)
{
    ...
    VP_INFO(VpDevCtxType, pDevCtx, ("VpSetDevTimerInt: %s %u ", (enable ? "Enable" : "Disable"), duration));
...
#ifdef VP_CC_MELT792_SERIES
        case VP_DEV_MELT792_SERIES:
            Melt792SetDeviceTimer(pDevCtx, enable, duration);
            break;
#endif /* VP_CC_MELT792_SERIES */
        default:
...
#endif /* defined (VP_CC_886_SERIES) || defined (VP_CC_MELT792_SERIES) */
