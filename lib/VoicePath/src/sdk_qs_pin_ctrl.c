 ...
 * All SM an DIN pins have a default behaviour as an input (interrupts) or an 
 * output (chip selects, clocks etc.). The default behaviour can be overridden by using 
 ...
 * Pins that can be configured as inputs are tri-stated.
 *
 * Example usage: Override ZTAP DIN connector PCLK signal to a low value instead of a
 *                square wave clock:
 *               
 *                PinControlSet(DIN_PCLK, PIN_CTRL_OUTPUT_LOW);
 *                PinControlEnable();
 * 
 * Applications that enable the pin control feature MUST also disable the feature if
 * the user expects the ZTAP to function properly after the application completes.
 *
 ...
 *
 *         a tri-stated input (PIN_CTRL_INPUT_TRI)
 ...
 *     ioLogic -    Logic behaviour
 *
 * Description: This function overrides a pins behaviour. The requested behaviour only takes
