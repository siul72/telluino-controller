 ...
 *
 * $Revision: 12593 $
 * $LastChangedDate: 2018-07-05 12:30:33 -0500 (Thu, 05 Jul 2018) $
 */
    ...
    bool adcScale)
{
    ...
    if (adcScale) {
        ...
        tmpResult = (int16)VpRoundedDivide(((int32)result + (int32)offset) * (int32)gain, 1000L);
    ...
    uint8 numRoutes)
{
    ...
    for (route = 0 ; route < numRoutes ; route++) {
        ...
        if (adcScale) {
            ...
            tmpResult = (int16)VpRoundedDivide(((int32)result[route] + (int32)offset) * (int32)gain, 1000L);
    ...
    VpDeviceCalType mode)
{
    ...
    if (!Vp886ReadyStatus(VP_NULL, pLineCtx, &status)) {
        Vp886ExitCritical(VP_NULL, pLineCtx, "Vp886CalCodec");
        return status;
    }
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    if (VP886_IS_ABS(pDevObj)) {
        ...
        /* Apply the battery switch calibration to both polarities */
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    /* The ABS DC profile could require an adjustment to the 60V clamp to apply
       or remove mid battery compensation */
    if (VP886_IS_ABS(pDevObj)) {
        diff = Vp886ComputeError(pLineCtx, VP886_ERR_60V_LONG, -60, VP_POL_NONE);
        Vp886AdjustCalReg(pLineCtx, pLineObj->registers.indCal, VP886_ERR_60V_LONG, diff, VP_POL_NONE);
    }
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    for (channel = 0; channel < pDevObj->staticInfo.maxChannels; channel++) {
        ...
        /* Save ioDir */
        VpSlacRegRead(pDevCtx, NULL, VP886_R_IODIR_RD, VP886_R_IODIR_LEN,
    ...
    switch (pDevObj->calCodecSubState[channelId]) {
        ...
        case VP886_LONG_GET_MEAS:
            ...
            if ((ABS(result[3]) > 1069) || (VP886_REVISION(pDevObj) == VP886_R_RCNPCN_RCN_AAA)) {
                ...
                VP_CALIBRATION(VpDevCtxType, pDevCtx,
                    ("Vp886LongitudinalCalibration(ch:%d): feed offset = %d mV, tip %d, ring %d, batt %d",
                    channelId, pCalData->longActive.offset, result[0], result[1], result[2]));
            }
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    switch (pLineObj->calLineSubState) {
        ...
        case VP886_RINGING_HIGH:
            ...
            VP_CALIBRATION(VpDevCtxType, pDevCtx,
                ("Vp886RingingCalibration(ch:%d): 0V result = %d, 90V result = %d",
                channelId, pDevObj->tmpResultA[channelId], result));
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    switch (pLineObj->calLineSubState) {
        ...
        case VP886_GND_KEY_COMPUTE:
            ...
            offset = (int16)VpRoundedDivide((ABS(pDevObj->tmpResultA[channelId]) + ABS(result)) * 3634L, 1000L) - 24000L;
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    switch (pLineObj->calLineSubState) {
        ...
        case VP886_ABS_BSW_HIGH:
            ...
            VP_CALIBRATION(VpDevCtxType, pDevCtx,
                ("Vp886ABSCalibration(ch:%d|pol:%d): offset = %i mV",
    ...
    uint8 polarity)
{
    ...
    VP_CALIBRATION(VpLineCtxType, pLineCtx,
        ("Vp886ApplyCalTrackingBat: voc %i, vas %i, ila %i, irOverhead %i", voc, vas, ila, irOverhead));
    ...
    VP_CALIBRATION(VpLineCtxType, pLineCtx,
        ("Vp886ApplyCalTrackingBat(pol:%d): Onhook VAS error = %i mV (target = %i, adjustedBat = %i uA)",
        polarity, diff, target, adjustedBat));
    steps = (int16)VpRoundedDivide(diff, VP886_VAS_CORR_STEP);
    if (steps > 7) {
        VP_CALIBRATION(VpLineCtxType, pLineCtx,
    ...
    VP_CALIBRATION(VpLineCtxType, pLineCtx, ("Vp886ApplyCalTrackingBat: Offhook VAS - vab %i", vab));
    ...
    diff = adjustedBat - target;
    VP_CALIBRATION(VpLineCtxType, pLineCtx, ("Vp886ApplyCalTrackingBat(pol:%d): Offhook VAS error = %i mV (target = %i, adjustedBat = %i mV)",
        polarity, diff, target, adjustedBat));
    steps = (int16)VpRoundedDivide(diff, VP886_VAS_CORR_STEP);
    if (steps > 7) {
    ...
    VP_CALIBRATION(VpLineCtxType, pLineCtx, ("Vp886ApplyCalTrackingBat(pol:%d): Long Pt error = %i mV",
        polarity, diff));

    if (VP886_REVISION(pDevObj) == VP886_R_RCNPCN_RCN_AAA) {
        steps = (int16)VpRoundedDivide(diff, VP886_LONG_CORR_STEP_REV1);
    } else {
        steps = (int16)VpRoundedDivide(diff, VP886_LONG_CORR_STEP_REV2);
    }
    ...
    uint8 *ringCal)
{
    ...
    VP_CALIBRATION(VpLineCtxType, pLineCtx,
        ("Vp886ApplyCalTrackingBatRinging: Bias %i, Amp %i, vas %i, vab %i", bias, amplitude, vas, vab));
    ...
    VP_CALIBRATION(VpLineCtxType, pLineCtx, ("Vp886ApplyCalTrackingBatRinging: Long Pt error = %i mV",
        diff));
    ...
    uint8 polarity)
{
    ...
    switch (errorType) {
        case VP886_ERR_ILA: {
            ...
            VP_CALIBRATION(VpLineCtxType, pLineCtx,
                ("Vp886ComputeError(pol:%d): ILA error = %i uA (actual = %i uA)",
        ...
        case VP886_ERR_VOC: {
            ...
            VP_CALIBRATION(VpLineCtxType, pLineCtx,
                ("Vp886ComputeError(pol:%d): VOC error = %i mV (actual = %i mV).  buffErr = %d",
        ...
        case VP886_ERR_HOOK: {
            ...
            VP_CALIBRATION(VpLineCtxType, pLineCtx,
                ("Vp886ComputeError(pol:%d): Hook threshold error = %i uA", polarity, diff));
            break;
        }
        case VP886_ERR_GND_KEY: {
            ...
            VP_CALIBRATION(VpLineCtxType, pLineCtx,
                ("Vp886ComputeError(pol:%d): Ground Key threshold error = %i uA", polarity, diff));
            break;
        }
        case VP886_ERR_RINGBIAS: {
            ...
            VP_CALIBRATION(VpLineCtxType, pLineCtx,
                ("Vp886ComputeError: Ringing DC Bias error = %i mV (actual = %i mV).  buffErr = %d",
                diff, actual, buffErr));
            break;
        }
        case VP886_ERR_FLOOR:
            ...
            VP_CALIBRATION(VpLineCtxType, pLineCtx,
                ("Vp886ComputeError: FloorV error = %i mV (actual = %i mV)",
                diff, actual));
            break;
        ...
        case VP886_ERR_FIXEDRING:
            ...
            diff = actual - (int32)target * 1000;
            VP_CALIBRATION(VpLineCtxType, pLineCtx,
                ("Vp886ComputeError: Fixed Ring Bat error = %i mV (actual = %i mV)",
                diff, actual));
            break;
        ...
        case VP886_ERR_LOWPOWER:
            ...
            diff = actual - (int32)target * 1000;
            VP_CALIBRATION(VpLineCtxType, pLineCtx,
                ("Vp886ComputeError: Low Power Batt error = %i mV (actual = %i mV, target = %d V)",
                diff, actual, target));
            break;
    ...
    uint8 polarity)
{
    ...
    switch (errorType) {
        case VP886_ERR_ILA:
            ...
            steps = (int16)VpRoundedDivide(diff, VP886_ILA_CORR_STEP);
            steps *= -1;
        ...
        case VP886_ERR_VOC:
            /* Compute the number of calibration increments (rounded) */
            steps = (int16)VpRoundedDivide(diff, VP886_VOC_CORR_STEP);
            steps *= -1;
        ...
        case VP886_ERR_HOOK:
            ...
            if (polarity == VP_POL_NONE) {
                steps = (int16)VpRoundedDivide(diff, VP886_HOOK_CORR_STEP_LP);
            } else {
                steps = (int16)VpRoundedDivide(diff, VP886_HOOK_CORR_STEP);
            }
        ...
        case VP886_ERR_GND_KEY:
            /* Compute the number of calibration increments (rounded) */
            steps = (int16)VpRoundedDivide(diff, VP886_GND_KEY_CORR_STEP);
            steps *= -1;
        ...
        case VP886_ERR_FLOOR:
            /* Compute the number of calibration increments (rounded) */
            steps = (int16)VpRoundedDivide(diff, VP886_FIXEDBAT_CORR_STEP);
            steps *= -1;
        ...
        case VP886_ERR_60V_LONG:
            ...
            if (VP886_IS_ABS(pDevObj) && pLineObj->midBatCompEnable == TRUE) {
                /* Apply mid battery compensation from DC profile */
                calReg[1] |= (pLineObj->midBatCompValue & VP886_R_INDCAL_60VCLAMP);
            } else {
                /* Apply a fixed 1 step toward ground longitudinal correction */
                calReg[1] |= 0x01;
            }
        ...
        case VP886_ERR_BAT_SAT:
            /* Compute the number of calibration increments (rounded) */
            steps = (int16)VpRoundedDivide(diff, VP886_BAT_SAT_CORR_STEP);
        ...
        case VP886_ERR_BAT_LIMIT:
            /* Compute the number of calibration increments (rounded) */
            steps = (int16)VpRoundedDivide(diff * -1, VP886_BAT_LIM_CORR_STEP);
