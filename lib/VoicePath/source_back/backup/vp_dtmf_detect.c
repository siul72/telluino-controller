    ...
    int32 sample)
{
    ...
    /* Skip this filter if the gain is set to 0 */
    if (pGoertzel->gainSqd == 0) {
        return;
    }
    ...
    bool freshStart)
{
    ...
    pDtmfData->grtzlLowRowGap.v2 = 0;
    pDtmfData->grtzlLowRowGap.v3 = 0;
    pDtmfData->grtzlLowRowGap.scale = 0;
    pDtmfData->grtzlHighRowGap.v2 = 0;
    pDtmfData->grtzlHighRowGap.v3 = 0;
    pDtmfData->grtzlHighRowGap.scale = 0;
    pDtmfData->grtzlLowColGap.v2 = 0;
    pDtmfData->grtzlLowColGap.v3 = 0;
    pDtmfData->grtzlLowColGap.scale = 0;
    ...
    VpDtmfDetectDataType *pDtmfData)
{
    ...
    switch (pParams->sampleRate) {
        ...
        case VP_DTMF_SAMPLE_RATE_4KHZ:
            ...
            /* Gap frequency detectors, used to reject signals just below the
               lowest row frequency, between the row and column ranges, and
               just above the highest column frequency.
               These frequencies were picked to produce results equal to 1/6
               of what the nearest main detector would produce if the tone is
               2.75% off of nominal.  1/6 is based on the default relative peak
               rejection factor, and 2.75% targets the center of the 2% accept
               and 3.75% reject frequency thresholds.
               There is no need for a high column gap filter because the
               response of the 1633Hz filter goes so low at 3.5% deviation that
               it would reject based on totalEnergy or other factors. */
            pDtmfData->grtzlLowRowGap.coeff = 36410; /* 625 Hz */
            pDtmfData->grtzlHighRowGap.coeff = -2264; /* 1022 Hz */
            pDtmfData->grtzlLowColGap.coeff = -12179; /* 1119 Hz */
            ...
            pDtmfData->grtzlLowRowGap.size = VP_DTMF_BLOCK_SIZE_4KHZ_625;
            pDtmfData->grtzlHighRowGap.size = VP_DTMF_BLOCK_SIZE_4KHZ_1022;
            pDtmfData->grtzlLowColGap.size = VP_DTMF_BLOCK_SIZE_4KHZ_1119;
        ...
        case VP_DTMF_SAMPLE_RATE_8KHZ:
            ...
            /* Gap frequency detectors, used to reject signals just below the
               ...
               response of the 1633Hz filter goes so low at 3.5% deviation that
               it would reject based on totalEnergy or other factors. */
            pDtmfData->grtzlLowRowGap.coeff = 57798; /* 625 Hz */
            pDtmfData->grtzlHighRowGap.coeff = 45533; /* 1022 Hz */
            pDtmfData->grtzlLowColGap.coeff = 41814; /* 1119 Hz */
            ...
            pDtmfData->grtzlLowRowGap.size = VP_DTMF_BLOCK_SIZE_8KHZ_625;
            pDtmfData->grtzlHighRowGap.size = VP_DTMF_BLOCK_SIZE_8KHZ_1022;
            pDtmfData->grtzlLowColGap.size = VP_DTMF_BLOCK_SIZE_8KHZ_1119;
    ...
    pDtmfData->grtzlLowRowGap.gainSqd = pParams->gainLowRowGap;
    pDtmfData->grtzlHighRowGap.gainSqd = pParams->gainHighRowGap;
    pDtmfData->grtzlLowColGap.gainSqd = pParams->gainLowColGap;
    ...
    uint8 numSamples)
{
    ...
    uint32 energyLowRowGap = 0;
    uint32 energyHighRowGap = 0;
    uint32 energyLowColGap = 0;
    ...
    while (i < numSamples) {
        ...
        while (i < numSamples && pDtmfData->currentSample < pDtmfData->blockSize) {
            ...
            VpDtmfDetectGoertzelUpdate(pLineCtx, pDtmfData, &pDtmfData->grtzlLowRowGap, sample);
            VpDtmfDetectGoertzelUpdate(pLineCtx, pDtmfData, &pDtmfData->grtzlHighRowGap, sample);
            VpDtmfDetectGoertzelUpdate(pLineCtx, pDtmfData, &pDtmfData->grtzlLowColGap, sample);
        ...
        energyRow[0] = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlRow[0]);
        bestRow = 0;
        energyCol[0] = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlCol[0]);
        bestCol = 0;
        for (j = 1; j < 4; j++) {
            energyRow[j] = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlRow[j]);
            if (energyRow[j] > energyRow[bestRow]) {
                bestRow = j;
            }
            energyCol[j] = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlCol[j]);
        ...
        /* Calculate the energies of the gap frequencies */
        energyLowRowGap = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlLowRowGap);
        energyHighRowGap = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlHighRowGap);
        energyLowColGap = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlLowColGap);
        ...
        if (hit != VP_DIG_NONE || pDtmfData->prevHit != VP_DIG_NONE || pDtmfData->digitOutput != VP_DIG_NONE) {
            VP_DTMF_DETAIL(VpLineCtxType, pLineCtx, (
                ...
                "energyRows: %d %d %d %d\n"
                "energyCols: %d %d %d %d\n"
                "peakRow %d: %d - peakCol %d: %d\n"
                "energyLowRowGap %d - energyHighRowGap %d - energyLowColGap %d\n"
                "harmonicRow %d - harmonicCol %d - totalEnergy %u\n"
                "prevHit %d, hit %d\n"
                "threshold %d",
                ...
                energyLowRowGap, energyHighRowGap, energyLowColGap,
        ...
        if (hit != VP_DIG_NONE) {
            ...
            /* Also perform relative peak checks against the gap frequencies.
               ...
               row frequency. */
            if (bestRow == 0 &&
                (energyLowRowGap * pParams->relPeakRowFactor) / 10 > peakEnergyRow)
            {
                hit = VP_DIG_NONE;
                VP_DTMF_DETAIL(VpLineCtxType, pLineCtx, ("failed low row gap check"));
            }
            if (bestRow == 3 &&
                (energyHighRowGap * pParams->relPeakRowFactor) / 10 > peakEnergyRow)
            {
                hit = VP_DIG_NONE;
                VP_DTMF_DETAIL(VpLineCtxType, pLineCtx, ("failed high row gap check"));
            }
            if (bestCol == 0 &&
                (energyLowColGap * pParams->relPeakColFactor) / 10 > peakEnergyCol)
            {
                hit = VP_DIG_NONE;
                VP_DTMF_DETAIL(VpLineCtxType, pLineCtx, ("failed low col gap check"));
            }
