...
static void
Vp886ManageABSRingingBattVoltage(
    VpDevCtxType *pDevCtx,
    bool programCalRegistersCh0,
    bool programCalRegistersCh1);

static void
Vp886ManageABSRingingBattSwitch(
    VpDevCtxType *pDevCtx);
    ...
    VpLineStateType state)
{
...
#ifdef VP886_INCLUDE_ADAPTIVE_RINGING
    /* disable ringing adaptive ringing if the mode and line not configuration do not make sense */
    if (((state == VP_LINE_RINGING) || (state == VP_LINE_RINGING_POLREV))   &&
        ...
        ((pCache->adaptiveRinging.mode  == VP_ADAPT_RING_SINGLE_BB_TRACKER) ||
         (pCache->adaptiveRinging.mode  == VP_ADAPT_RING_FIXED_TRACKER))    &&
        !(pLineObj->registers.swParam[0] & VP886_R_SWPARAM_RING_TRACKING)) {

            VP_ERROR(VpLineCtxType, pLineCtx, ("Cannot enter adaptive ringing: ring profile is tracking"));
            pDevObj->options.adaptiveRinging.power = VP_ADAPTIVE_RINGING_DISABLED;
    }
    ...
    VpLineStateType state)
{
    ...
    if (hookFreezeMs > 0) {
        VP_HOOK(VpLineCtxType, pLineCtx, ("Hook freeze for %ums", hookFreezeMs));
    ...
    VpLineStateType newState)
{
    ...
    /* If the line state is changed out of disconnect while the ICR settings
       have been changed for VpGetLoopCond(), restore the cached settings first */
    if (pLineObj->getLoopCondRegsCached &&
        prevState == VP_LINE_DISCONNECT &&
        newState != VP_LINE_DISCONNECT)
    {
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    /* Only change the power mode for SINGLE configurations.  MASTER and SLAVE
       should remain as initialized. */
    ...
    bool ringExit)
{
    ...
    /* If managing BBABS, use the user line state.  Otherwise (for 3-battery
       mode) use the true line state, to move out of high battery during
       ring-off periods. */
...
#ifdef VP_CSLAC_SEQ_EN
    /* If message waiting pulse is active and the requested voltage magnitude is
       greater than VOC, force high battery levels for BBABS.
       For 3-battery mode high battery is also necessary during message waiting
       pulses regardless of the requested voltage due to VBH being used for
       the longitudinal point.  Even a low voltage pulse could fail if we don't
       switch to VBH.  Since the MW pulse uses the ringing state, just allow it
       to be handled by the switch statement below instead of here for non-BBABS. */
    if (pDevObj->devProfileData.swCfg == VP886_DEV_PROFILE_SW_CONF_BB &&
        pLineObj->sendSignal.type == VP_SENDSIG_MSG_WAIT_PULSE &&
        pLineObj->sendSignal.active == TRUE &&
        ABS(pLineObj->sendSignal.msgWait.voltage) > pLineObj->targetVoc)
    {
        if (pDevObj->absRingingBattRequired[channelId] == FALSE) {
            pDevObj->absRingingBattRequired[channelId] = TRUE;
            VP_LINE_STATE(VpDevCtxType, pDevCtx, ("Setting absRingingBattRequired for ch%d, mw pulse %dV",
                channelId, pLineObj->sendSignal.msgWait.voltage));
        }
        return;
    }
#endif
    ...
    switch (state) {
        ...
        case VP_LINE_OHT:
        case VP_LINE_OHT_POLREV:
            /* For BBABS, leave the flag alone when changing to a state that is
               normally used between ringing bursts, so that the battery isn't
               being changed repeatedly during manual ring cadencing.
               Otherwise (for 3-battery mode) clear the flag to reduce power
               consumption during the ring-off times. */
            if (ringExit && pDevObj->devProfileData.swCfg == VP886_DEV_PROFILE_SW_CONF_BB) {
    ...
    bool programCalRegistersCh1)
{
    ...
    /* Manage battery voltage for BBABS */
    if (pDevObj->devProfileData.swCfg == VP886_DEV_PROFILE_SW_CONF_BB) {
        Vp886ManageABSRingingBattVoltage(pDevCtx, programCalRegistersCh0, programCalRegistersCh1);
    }
    
    /* Manage GPIO battery switch for 3-battery designs */
    if (pDevObj->devProfileData.batSelGpio != VP886_DEV_PROFILE_BAT_SEL_GPIO_NONE) {
        Vp886ManageABSRingingBattSwitch(pDevCtx);
    }
    ...
    bool programCalRegistersCh1)
{
    ...
    if (pDevObj->stateInt & VP886_BATTERY_BACKUP) {
        normalY = pDevObj->battBackup.swy;
        vbhOverhead = pDevObj->battBackup.vbhOverhead;
    } else {
        normalY = pDevObj->devProfileData.swyv;
        vbhOverhead = pDevObj->devProfileData.vbhOverhead;
    }
    normalZ = pDevObj->devProfileData.swzv;

    maxRingPeak = 0;
    
    /* Find the highest required ringing battery voltage, among the lines that
       are currently in ringing state or message waiting pulse (high power
       required) */
    for (channelId = 0; channelId < 2; channelId++) {
        if (pDevObj->absRingingBattRequired[channelId] == TRUE) {
            bool usingMwPulse = FALSE;
#ifdef VP_CSLAC_SEQ_EN
            ...
            if (pLineCtx != VP_NULL) {
                ...
                if (pLineObj->sendSignal.type == VP_SENDSIG_MSG_WAIT_PULSE &&
                    pLineObj->sendSignal.active == TRUE &&
                    ABS(pLineObj->sendSignal.msgWait.voltage) > maxRingPeak)
                {
                    maxRingPeak = ABS(pLineObj->sendSignal.msgWait.voltage);
                    usingMwPulse = TRUE;
                }
            }
#endif
    ...
    if (pLineCtx != VP_NULL) {
        ...
        swParamBatVal = (uint8)VpRoundedDivide(pDevObj->swyv - 5, 5);
    ...
    if (pLineCtx != VP_NULL) {
        ...
        pLineObj = pLineCtx->pLineObj;
        pLineObj->registers.swParam[0] &= ~VP886_R_SWPARAM_ABS_V;
        swParamBatVal = (uint8)VpRoundedDivide(pDevObj->swzv - 5, 5);
        pLineObj->registers.swParam[0] |= swParamBatVal;
        VpSlacRegWrite(NULL, pLineCtx, VP886_R_SWPARAM_WRT, VP886_R_SWPARAM_LEN, pLineObj->registers.swParam);
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    if (pDevObj->absRingingBattRequired[0] == TRUE ||
        pDevObj->absRingingBattRequired[1] == TRUE)
    {
        ioDataState = 1;
    } else {
        ioDataState = 0;
    ...
    /* Do nothing if the IO state isn't changing */
    if (ioDataState == pDevObj->batSelGpioState) {
        return;
    }
    ...
    switch (pDevObj->devProfileData.batSelGpio) {
        ...
        case VP886_DEV_PROFILE_BAT_SEL_GPIO_IO22:
            ...
            break;
        default:
            VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886ManageABSRingingBattSwitch - Unknown batSelGpio %d\n", pDevObj->devProfileData.batSelGpio));
            return;
    }
    
    pLineCtx = pDevCtx->pLineCtx[channelId];
    if (pLineCtx == VP_NULL) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886ManageABSRingingBattSwitch - Line context %d NULL\n", channelId));
        return;
    }
    
    VpSlacRegRead(NULL, pLineCtx, VP886_R_IODATA_RD, VP886_R_IODATA_LEN, ioDataReg);
    ioDataReg[0] &= ~ioDataMask;
    ioDataReg[0] |= (ioDataState << ioDataShift);
    VpSlacRegWrite(NULL, pLineCtx, VP886_R_IODATA_WRT, VP886_R_IODATA_LEN, ioDataReg);

    VP_LINE_STATE(VpDevCtxType, pDevCtx, ("Vp886ManageABSRingingBattSwitch - Setting %s battery, ch%d 0x%02X\n",
        (ioDataState == 0 ? "MED" : "HIGH"), channelId, ioDataReg[0]));

    pDevObj->batSelGpioState = ioDataState;
    ...
    VpLineStateType state)
{
    ...
    Vp886LineStateInfoType otherLineStateUserInfo;
    Vp886LineStateInfoType lineStateUserInfo;
    ...
    otherLineStateUserInfo = Vp886LineStateInfo(pOtherLineObj->lineState.usrCurrent);
    lineStateUserInfo = Vp886LineStateInfo(pLineObj->lineState.usrCurrent);

    if (otherLineStateInfo.normalEquiv != VP_LINE_RINGING ||
        otherLineStateUserInfo.normalEquiv != VP_LINE_RINGING ||
        lineStateUserInfo.normalEquiv != VP_LINE_RINGING) {
        /* No need to do anything if the other line isn't ringing.
           Checking the user-state too to avoid running this in other cases
           where the ringing state may be used internally, such as during a
           message waiting pulse cadence. */
        return FALSE;
    }
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    VP_LINE_STATE(VpLineCtxType, pOtherLineCtx, ("Vp886RingSyncFinish() delaying ring entry by %uus", delay_us));
    ...
    if (VP886_IS_SF(pDevObj)) {
        ...
        delayReg = (uint8)VpRoundedDivide(delay_us, 250);
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    VP_GAIN(VpLineCtxType, pLineCtx, ("Post Multiplied gxInt (%d) grInt (%d)",
        gxInt, grInt));
    ...
    VP_GAIN(VpLineCtxType, pLineCtx, ("Post Range Check gxInt (%d) grInt (%d)",
        gxInt, grInt));
    ...
    void *pValue)
{
    ...
    if (option != VP_OPTION_ID_DEBUG_SELECT &&
        option != VP_DEVICE_OPTION_ID_ACCESS_CTRL &&
        option != VP_DEVICE_OPTION_ID_SPI_ERROR_CTRL) {
    ...
    switch (option) {
        ...
        case VP_DEVICE_OPTION_ID_ADAPTIVE_RINGING: {
            ...
            if (pUserBuf->validMask & VP_ADAPTIVE_RINGING_CFG_MODE) {
                ...
                if (pCache->adaptiveRinging.power != VP_ADAPTIVE_RINGING_DISABLED) {
                    ...
                    } else if ((pUserBuf->mode == VP_ADAPT_RING_FIXED_TRACKER) && VP886_IS_TRACKER(pDevObj) &&
                        (pDevObj->devProfileData.swCfg >= VP886_DEV_PROFILE_SW_CONF_FB100) &&
                        (pDevObj->devProfileData.swCfg <= VP886_DEV_PROFILE_SW_CONF_IB100_5IN)) {
                        pCache->adaptiveRinging.mode = VP_ADAPT_RING_FIXED_TRACKER;

                    } else if ((pUserBuf->mode == VP_ADAPT_RING_FULL_TRACKER) && VP886_IS_TRACKER(pDevObj) &&
                        (pDevObj->devProfileData.swCfg >= VP886_DEV_PROFILE_SW_CONF_FB100) &&
                        (pDevObj->devProfileData.swCfg <= VP886_DEV_PROFILE_SW_CONF_IB100_5IN)) {
                        pCache->adaptiveRinging.mode = VP_ADAPT_RING_FULL_TRACKER;
        ...
        case VP_DEVICE_OPTION_ID_ACCESS_CTRL: {
            VpOptionAccessCtrlType *pUserBuf = pValue;
            pCache->accessCtrl = *pUserBuf;
            break;
        }
        case VP_DEVICE_OPTION_ID_SPI_ERROR_CTRL: {
            VpOptionSpiErrorCtrlType *pUserBuf = pValue;
            pCache->spiErrorCtrl = *pUserBuf;
            /* Reset the count and blocking flag */
    ...
    switch (option) {
        ...
        case VP_OPTION_ID_LOOPBACK: {
            ...
            switch(*pUserBuf) {
                ...
                case VP_OPTION_LB_TIMESLOT_WITH_DAC:
                    pLineObj->registers.opCond[0] |= VP886_R_OPCOND_TSA_LOOPBACK;
                    
                    /* ensure the device has control of the DAC */
                    pLineObj->registers.icr4[0] &= ~VP886_R_ICR4_VDAC_EN;
                    break;
        ...
        case VP_OPTION_ID_DTMF_PARAMS: {
            ...
            if (pUserBuf->validMask & VP_OPTION_CFG_MIN_DETECT) {
                if (pUserBuf->minDetect > 0 || pUserBuf->minDetect < -535) {
                    VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionLine - minDetect out of range (%d)",
                        pUserBuf->minDetect));
                    return VP_STATUS_INVALID_ARG;
                }
                ...
                VP_DTMF(VpLineCtxType, pLineCtx, ("Vp886SetOptionLine - minDetect %d -> basicThreshold %d",
            ...
            if (pUserBuf->validMask & VP_OPTION_CFG_ROW_TO_COL_LIMIT) {
                if (pUserBuf->rowToColLimit < 0 || pUserBuf->rowToColLimit > 700) {
                    VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionLine - rowToColLimit out of range (%d)",
                ...
                pParams->normalTwistFactor =  (uint16)VpRoundedDivide(factor, 10);
            }
            if (pUserBuf->validMask & VP_OPTION_CFG_COL_TO_ROW_LIMIT) {
                if (pUserBuf->colToRowLimit < 0 || pUserBuf->colToRowLimit > 700) {
                    VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionLine - colToRowLimit out of range (%d)",
                ...
                pParams->reverseTwistFactor =  (uint16)VpRoundedDivide(factor, 10);
            }
            break;
        }
#endif /* VP886_INCLUDE_DTMF_DETECT */
    ...
    uint8 *pIoDirReg)
{
    ...
    if (pDevObj->ioCapability == VP886_IO_CAPABILITY_TWO_PER_CH) {
        ...
        *pIoDirReg = VP886_R_IODIR_IOD1_INPUT;
        ...
        /* Check if this I/O is configured as battery select GPIO.  If so, it
           must be set as a driven output. */
        if ((pDevObj->devProfileData.batSelGpio == VP886_DEV_PROFILE_BAT_SEL_GPIO_IO11 &&
             channelId == 0) ||
            (pDevObj->devProfileData.batSelGpio == VP886_DEV_PROFILE_BAT_SEL_GPIO_IO12 &&
             channelId == 1))
        {
            if (dirBit != VP_IO_OUTPUT_PIN || typeBit != VP_OUTPUT_DRIVEN_PIN) {
                VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886SetOptionIoCfg() ch%d I/O 1 reserved for battery select", channelId));
                status = VP_STATUS_DEDICATED_PINS;
                dirBit = VP_IO_OUTPUT_PIN;
                typeBit = VP_OUTPUT_DRIVEN_PIN;
            }
        }

        /* If the direction bit is set ... */
        if (dirBit == VP_IO_OUTPUT_PIN) {
            /* I/O 1 is an output pin */
            /* If the type bit is not set ... */
            if (typeBit == VP_OUTPUT_DRIVEN_PIN) {
                /* I/O 1 is a digital output */
                *pIoDirReg |= VP886_R_IODIR_IOD1_DIG_OUTPUT;
            } else { /* VP_OUTPUT_OPEN_PIN */
        ...
        /* Check if this I/O is configured as battery select GPIO.  If so, it
           must be set as a driven output. */
        if ((pDevObj->devProfileData.batSelGpio == VP886_DEV_PROFILE_BAT_SEL_GPIO_IO21 &&
             channelId == 0) ||
            (pDevObj->devProfileData.batSelGpio == VP886_DEV_PROFILE_BAT_SEL_GPIO_IO22 &&
             channelId == 1))
        {
            if (dirBit != VP_IO_OUTPUT_PIN || typeBit != VP_OUTPUT_DRIVEN_PIN) {
                VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886SetOptionIoCfg() ch%d I/O 2 reserved for battery select", channelId));
                status = VP_STATUS_DEDICATED_PINS;
                dirBit = 0;
                typeBit = 0;
            }
        ...
        *pIoDirReg = VP886_R_IODIR_IOD1_DIG_OUTPUT;
        ...
        /* Check if this I/O is configured as battery select GPIO.  If so, it
           must be set as a driven output. */
        if (pDevObj->devProfileData.batSelGpio == VP886_DEV_PROFILE_BAT_SEL_GPIO_IO21) {
            if (!(direction & 0x1) || (outputType & 0x1)) {
                VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886SetOptionIoCfg() ch%d I/O reserved for battery select", channelId));
                status = VP_STATUS_DEDICATED_PINS;
                direction |= 0x1;
                outputType &= ~0x1;
            }
    ...
    VpOptionDcFeedParamsType *pDcFeedParams)
{
    ...
    if (pDcFeedParams->validMask & VP_OPTION_CFG_VOC) {
        ...
        if (pDcFeedParams->voc < VP886_VOC_MIN || pDcFeedParams->voc > VP886_VOC_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionDcFeedParams - voc out of range (%d)",
                pDcFeedParams->voc));
            return VP_STATUS_INVALID_ARG;
        }
        voc = (uint8)VpRoundedDivide(pDcFeedParams->voc - VP886_VOC_OFFSET, VP886_VOC_STEP);
        if (voc > 0x7) {
            voc -= 0x8;
            vocsft = 0;
        } else {
        ...
        vocTarget = (uint8)VpRoundedDivide(pDcFeedParams->voc, VP886_VOC_TARGET_STEP);
        ...
        if (VP886_IS_TRACKER(pDevObj)) {
            ...
            lpBatt = (uint8)VpRoundedDivide(vocTarget + 4, 5) - 1;
        }
    }
    ...
    if (pDcFeedParams->validMask & VP_OPTION_CFG_ILA) {
        ...
        if (pDcFeedParams->ila < VP886_ILA_MIN || pDcFeedParams->ila > VP886_ILA_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionDcFeedParams - ila out of range (%d)",
                pDcFeedParams->ila));
            return VP_STATUS_INVALID_ARG;
        }
        ila = (uint8)VpRoundedDivide(pDcFeedParams->ila - VP886_ILA_OFFSET, VP886_ILA_STEP);
    }
    ...
    if (pDcFeedParams->validMask & VP_OPTION_CFG_HOOK_THRESHOLD) {
        ...
        if (pDcFeedParams->hookThreshold < VP886_TSH_MIN || pDcFeedParams->hookThreshold > VP886_TSH_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionDcFeedParams - hookThreshold out of range (%d)",
                pDcFeedParams->hookThreshold));
            return VP_STATUS_INVALID_ARG;
        }
        tsh = (uint8)VpRoundedDivide(pDcFeedParams->hookThreshold - VP886_TSH_OFFSET, VP886_TSH_STEP);
    }
    ...
    if (pDcFeedParams->validMask & VP_OPTION_CFG_LP_HOOK_THRESHOLD) {
        ...
        if (pDcFeedParams->lpHookThreshold < VP886_LPTSH_MIN || pDcFeedParams->lpHookThreshold > VP886_LPTSH_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionDcFeedParams - lpHookThreshold out of range (%d)",
                pDcFeedParams->lpHookThreshold));
            return VP_STATUS_INVALID_ARG;
        }
        lptsh = (uint8)VpRoundedDivide(pDcFeedParams->lpHookThreshold - VP886_LPTSH_OFFSET, VP886_LPTSH_STEP);
    }
    ...
    if (pDcFeedParams->validMask & VP_OPTION_CFG_GKEY_THRESHOLD) {
        ...
        if (pDcFeedParams->gkeyThreshold < VP886_TGK_MIN || pDcFeedParams->gkeyThreshold > VP886_TGK_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionDcFeedParams - gkeyThreshold out of range (%d)",
                pDcFeedParams->gkeyThreshold));
            return VP_STATUS_INVALID_ARG;
        }
        tgk = (uint8)VpRoundedDivide(pDcFeedParams->gkeyThreshold - VP886_TGK_OFFSET, VP886_TGK_STEP);
    }
        ...
        (pDcFeedParams->validMask & VP_OPTION_CFG_BATT_FLOOR))
    {
        ...
        if (pDcFeedParams->battFloor < VP886_BATTFLR_MIN || pDcFeedParams->battFloor > VP886_BATTFLR_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionDcFeedParams - battFloor out of range (%d)",
                pDcFeedParams->battFloor));
            return VP_STATUS_INVALID_ARG;
        }
        battFloor = (uint8)VpRoundedDivide(pDcFeedParams->battFloor - VP886_BATTFLR_OFFSET, VP886_BATTFLR_STEP);
    }
    ...
    VpOptionRingingParamsType *pRingingParams)
{
    ...
    if (pRingingParams->validMask & VP_OPTION_CFG_FREQUENCY) {
        if (!trapezoidal) {
            ...
            if (pRingingParams->frequency < VP886_FREQ_MIN || pRingingParams->frequency >= VP886_FREQ_MAX) {
                VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionRingingParams - frequency out of range (%d)",
    ...
    if (pRingingParams->validMask & VP_OPTION_CFG_AMPLITUDE) {
        ...
        if (pRingingParams->amplitude < VP886_AMP_MIN || pRingingParams->amplitude >= VP886_AMP_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionRingingParams - amplitude out of range (%d)",
    ...
    if (pRingingParams->validMask & VP_OPTION_CFG_DC_BIAS) {
        ...
        if (pRingingParams->dcBias < VP886_BIAS_MIN || pRingingParams->dcBias > VP886_BIAS_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionRingingParams - dcBias out of range (%d)",
    ...
    if (pRingingParams->validMask & VP_OPTION_CFG_RINGTRIP_THRESHOLD) {
        ...
        if (pRingingParams->ringTripThreshold < VP886_RTTH_MIN || pRingingParams->ringTripThreshold > VP886_RTTH_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionRingingParams - ringTripThreshold out of range (%d)",
                pRingingParams->ringTripThreshold));
            return VP_STATUS_INVALID_ARG;
        }
        rtth = (uint8)VpRoundedDivide(pRingingParams->ringTripThreshold - VP886_RTTH_OFFSET, VP886_RTTH_STEP);
    }
    ...
    if (pRingingParams->validMask & VP_OPTION_CFG_RING_CURRENT_LIMIT) {
        ...
        if (pRingingParams->ringCurrentLimit < VP886_ILR_MIN || pRingingParams->ringCurrentLimit > VP886_ILR_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionRingingParams - ringCurrentLimit out of range (%d)",
        ...
        if (pRingingParams->ringCurrentLimit > VP886_ILR_LOW_MAX) {
            /* Ring current limit is a 5-bit value with a 50-112 mA scale (2 mA per
               step + 50 mA offset). */
            ilr = (uint8)VpRoundedDivide(pRingingParams->ringCurrentLimit - VP886_ILR_OFFSET, VP886_ILR_STEP);
            ...
            ilr = (uint8)VpRoundedDivide(pRingingParams->ringCurrentLimit - VP886_ILR_LOW_OFFSET, VP886_ILR_LOW_STEP);
    ...
    if (trapezoidal && (pRingingParams->validMask & VP_OPTION_CFG_TRAP_RISE_TIME)) {
        ...
        if (pRingingParams->trapRiseTime <= VP886_TRAPRISE_MIN || pRingingParams->trapRiseTime > VP886_TRAPRISE_MAX) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionRingingParams - trapRiseTime out of range (%d)",
    ...
    /* Calculate the programmed amplitude and bias in mV.
       ...
       per 0x400). */
    if (pRingingParams->validMask & VP_OPTION_CFG_AMPLITUDE) {
        amplitude_mV = VpRoundedDivide(amplitude * VP886_AMP_STEP_NUM, VP886_AMP_STEP_DEN);
    } else {
        amplitude_mV = VpRoundedDivide(pLineObj->ringAmplitude * VP886_AMP_STEP_NUM, VP886_AMP_STEP_DEN);
    }
    if (pRingingParams->validMask & VP_OPTION_CFG_DC_BIAS) {
        bias_mV = VpRoundedDivide(bias * VP886_BIAS_STEP_NUM, VP886_BIAS_STEP_DEN);
    } else {
        bias_mV = VpRoundedDivide(pLineObj->ringBias * VP886_BIAS_STEP_NUM, VP886_BIAS_STEP_DEN);
    } 

    /* Error check the amplitude+bias.  Should not exceed the 154.4V max */
    if (ABS(amplitude_mV) + ABS(bias_mV) > VP886_AMP_MAX) {
        VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionRingingParams - amplitude+bias out of range (%d+%d = %d)",
            ABS(amplitude_mV), ABS(bias_mV), ABS(amplitude_mV) + ABS(bias_mV)));
        return VP_STATUS_INVALID_ARG;
    }
        ...
        (pLineObj->registers.swParam[0] & VP886_R_SWPARAM_RING_TRACKING) == VP886_R_SWPARAM_RING_TRACKING_DIS)
    {
        ...
        swrv = (uint8)VpRoundedDivide(battery_mV, 1000);
        if (swrv > 160) {
            swrv = 160;
        }
        ...
        pLineObj->refRingingBatV = ((pLineObj->registers.swParam[1] & VP886_R_SWPARAM_RINGING_V) * 5 + 5);
        VpSysDebugPrintf("Vp886SetOptionRingingParams: pLineObj->refRingingBatV %i, swParam[1] 0x%02X\n", pLineObj->refRingingBatV, pLineObj->registers.swParam[1]);
        VpSlacRegWrite(NULL, pLineCtx, VP886_R_SWPARAM_WRT, VP886_R_SWPARAM_LEN, pLineObj->registers.swParam);
    }
         ...
         (pRingingParams->validMask & VP_OPTION_CFG_DC_BIAS)))
    {
        ...
        pDevObj->absRingingPeak[channelId] = (uint8)((ABS(amplitude_mV) + ABS(bias_mV) + 999) / 1000);
    ...
    VpOptionDtmfModeType *pDtmfMode)
{
    ...
    switch (pDtmfMode->dtmfControlMode) {
        ...
        case VP_OPTION_DTMF_DECODE_ON: {
            ...
            pParams->relPeakRowFactor = VP886_DTMF_REL_PEAK_ROW_FACTOR;
            pParams->relPeakColFactor = VP886_DTMF_REL_PEAK_COL_FACTOR;
            pParams->secondHarmRowFactor = VP886_DTMF_2ND_HARM_ROW_FACTOR;
            pParams->secondHarmColFactor = VP886_DTMF_2ND_HARM_ROW_FACTOR;
            if (pLineObj->dtmf.useVadc) {
                pParams->totalEnergyFactor = VP886_DTMF_TOTAL_ENERGY_VADC_FACTOR;
            } else {
                pParams->totalEnergyFactor = VP886_DTMF_TOTAL_ENERGY_SADC_FACTOR;
            }
            ...
            if (pLineObj->dtmf.useVadc) {
                if (pParams->sampleRate == VP_DTMF_SAMPLE_RATE_4KHZ) {
                    ...
                    pParams->gainLowRowGap = 1000;  /* 625 Hz */
                    pParams->gainHighRowGap = 1000; /* 1022 Hz */
                    pParams->gainLowColGap = 1000;  /* 1119 Hz */
                } else {
                    /* 8kHz VADC */
                    pParams->gainRow[0] = 1000;     /* 697 Hz */
                    pParams->gainRow[1] = 1000;     /* 770 Hz */
                ...
                if (pParams->sampleRate == VP_DTMF_SAMPLE_RATE_4KHZ) {
                    ...
                    pParams->gain2ndCol[2] = 0;     /* 2954 Hz */
                    pParams->gain2ndCol[3] = 0;     /* 3266 Hz */
                    pParams->gainLowRowGap = 1275;  /* 625 Hz */
                    pParams->gainHighRowGap = 1927; /* 1022 Hz */
                    pParams->gainLowColGap = 2211;  /* 1119 Hz */
                } else {
                    /* 8kHz SADC */
                    ...
                    pParams->gainLowRowGap = 1062;  /* 625 Hz */
                    pParams->gainHighRowGap = 1176; /* 1022 Hz */
                    pParams->gainLowColGap = 1215;  /* 1119 Hz */
                }
            }
    ...
    VpDeviceIoAccessDataType *pDeviceIoData)
{
    ...
    /* If the operation is a WRITE, remove from the access mask any IO bits
       that are configured as inputs or reserved for battery switch control */
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    pDevObj->stateInt |= VP886_FREE_RUN;
...
/** Vp886BatteryBackupMode()
  Implements VpBatteryBackupMode() to apply or revert battery backup power
  supply settings.

  See the VP-API-II Reference Guide for more details on VpBatteryBackupMode().
*/
VpStatusType
Vp886BatteryBackupMode(
    VpDevCtxType *pDevCtx,
    ...
    uint8 vsw)
{
    ...
    Vp886EnterCritical(pDevCtx, VP_NULL, "Vp886BatteryBackupMode");

    if (!Vp886ReadyStatus(pDevCtx, VP_NULL, &status)) {
        Vp886ExitCritical(pDevCtx, VP_NULL, "Vp886BatteryBackupMode");
        return status;
    }

    if (backupMode == VP_BATT_BACKUP_ENABLE) {
        uint8 i;
        bool timingsValid = FALSE;

        pDevObj->stateInt |= VP886_BATTERY_BACKUP;

        /* Check if the backup timing params are all 0x00 or valid */
        for (i = 0; i < VP886_R_SWTIMING_LEN; i++) {
            if (pDevObj->swTimingParamsBattBackup[i] != 0x00) {
                timingsValid = TRUE;
                break;
            }
        }
        if (timingsValid) {
            /* Load the battery backup timing parameters */
            Vp886SetSwTimingParams(pDevCtx);
        }

        /* BBABS high battery voltage depends on the switcher input voltage vsw */
        if (VP886_IS_ABS(pDevObj) &&
            pDevObj->devProfileData.swCfg == VP886_DEV_PROFILE_SW_CONF_BB &&
            pDevObj->devProfileData.vsw != 0)
        {
            ...
            /* Calculate the vsw difference, rounding to nearest 1V */
            vswDiff = vsw - pDevObj->devProfileData.vsw;
            vswDiff = (uint8)VpRoundedDivide(vswDiff, 5);
            
            /* To maintain the same VBH, decrease VBL by 1V for each 2V increase
               in vsw (or vice versa) */
            pDevObj->battBackup.swy = pDevObj->devProfileData.swyv - (uint8)VpRoundedDivide(vswDiff, 2);
            
            /* vbhOverhead is defined as (desired overhead - vsw), so it needs
               to be recalculated when vsw changes */
            pDevObj->battBackup.vbhOverhead = pDevObj->devProfileData.vbhOverhead - vswDiff;
            
            /* Apply the new settings. */
            Vp886ManageABSRingingBatt(pDevCtx, TRUE, TRUE);
        }
        
        pDevObj->battBackup.vsw = vsw;
    } else { /* VP_BATT_BACKUP_DISABLE */
        /* Restore the original timing parameters */
        pDevObj->stateInt &= ~VP886_BATTERY_BACKUP;
        Vp886SetSwTimingParams(pDevCtx);

        /* Restore normal BBABS battery voltages */
        if (VP886_IS_ABS(pDevObj) &&
            pDevObj->devProfileData.swCfg == VP886_DEV_PROFILE_SW_CONF_BB &&
            pDevObj->devProfileData.vsw != 0)
        {
