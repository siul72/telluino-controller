 ...
 *
 * $Revision: 12588 $
 * $LastChangedDate: 2018-06-26 15:32:22 -0500 (Tue, 26 Jun 2018) $
 */
...
VpStatusType
Vp886QuerySwDutyCycleStart(
    VpLineCtxType *pLineCtx,
    uint16 handle);

VpStatusType
Vp886QueryTemperatureStart(
    VpLineCtxType *pLineCtx,
    uint16 handle);
    ...
    uint16 handle)
{
    ...
    /* If initialization has not been started or if SPI/ZSI communication is
       blocked, we can't push the response event. */
    if (!(pDevObj->busyFlags & (VP_DEV_INIT_CMP | VP_DEV_INIT_IN_PROGRESS))) {
        Vp886ExitCritical(pDevCtx, NULL, "Vp886GetOption");
        return VP_STATUS_DEV_NOT_INITIALIZED;
    }
    if (pDevObj->options.accessCtrl == VP_ACCESS_CTRL_BLOCK_DEVICE) {
        Vp886ExitCritical(pDevCtx, NULL, "Vp886GetOption");
        return VP_STATUS_ACCESS_BLOCKED;
    }
    ...
    switch (option) {
        ...
        case VP_DEVICE_OPTION_ID_ACCESS_CTRL:
        case VP_DEVICE_OPTION_ID_SPI_ERROR_CTRL:
            break;
        default:
            if (pLineCtx != VP_NULL) {
...
/** Vp886GetDeviceInfo()
  Implements VpGetDeviceInfo() to return information about a device.

  See the VP-API-II Reference Guide for more details on VpGetDeviceInfo().
*/
VpStatusType
Vp886GetDeviceInfo(
    VpDeviceInfoType *pDeviceInfo)
{
    ...
    Vp886EnterCritical(pDevCtx, VP_NULL, "Vp886GetDeviceInfo");

    /* If this is being called before VpInitDevice(), read the relevant
       registers for product and revision information */
    ...
    pDeviceInfo->deviceId = pDevObj->deviceId;
    pDeviceInfo->numLines = pDevObj->staticInfo.maxChannels;
    pDeviceInfo->revCode = pDevObj->staticInfo.rcnPcn[VP886_R_RCNPCN_RCN_IDX];
    pDeviceInfo->productCode = pDevObj->staticInfo.rcnPcn[VP886_R_RCNPCN_PCN_IDX];
    pDeviceInfo->featureList.testLoadSwitch = VP_NOT_AVAILABLE;
    pDeviceInfo->featureList.internalTestTermination = VP_AVAILABLE;

    /* Construct internal product code from fuse8 register */
    intPcn = (pDevObj->registers.fuse8[0] << 24);
    intPcn |= (pDevObj->registers.fuse8[1] << 16);
    intPcn |= (pDevObj->registers.fuse8[2] << 8);
    intPcn |= pDevObj->registers.fuse8[3];
    /* Mask out bits that are used for other things */
    intPcn &= 0xC0C0181F;

    pDeviceInfo->intProductCode = intPcn;

    Vp886ExitCritical(pDevCtx, VP_NULL, "Vp886GetDeviceInfo");
    ...
    uint16 handle)
{
    ...
    if ((pDevObj->options.adaptiveRinging.power != VP_ADAPTIVE_RINGING_DISABLED) && 
        ...
        VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886GetLoopCond() - Cannot run during adaptive ringing"));
    ...
    if (pLineObj->inLineTest) {
        VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886GetLoopCond() - Cannot run during line test"));
        Vp886ExitCritical(VP_NULL, pLineCtx, "Vp886GetLoopCond");
        return VP_STATUS_DEVICE_BUSY;
    }
#endif

    if (pLineObj->busyFlags & VP886_LINE_QUERY_TEMP) {
        VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886GetLoopCond() - Cannot run during VP_QUERY_ID_TEMPERATURE"));
        Vp886ExitCritical(VP_NULL, pLineCtx, "Vp886GetLoopCond");
        return VP_STATUS_DEVICE_BUSY;
    }

    if (pDevObj->getResultsRequired) {
        VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886GetLoopCond() - Waiting to clear previous read"));
        Vp886ExitCritical(VP_NULL, pLineCtx, "Vp886GetLoopCond");
        return VP_STATUS_DEVICE_BUSY;
    }
    ...
    if ((pLineObj->registers.sysState[0] & VP886_R_STATE_SS) == VP886_R_STATE_SS_DISCONNECT) {
        ...
        pLineObj->getLoopCondRegsCached = TRUE;
    }
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    /* Compute the loop resistance */
    if (imt != 0) {
        /* Use a full scale of 11.67kR (240 / 0.0595 / 11670 * 32768 = 11326) */
        if (ABS(vab * 11326L / imt) > VP_INT16_MAX) {
            pDevObj->basicResults.getLoopCond.rloop = VP_INT16_MAX;
        } else {
            pDevObj->basicResults.getLoopCond.rloop = (int16)ABS(vab * 11326L / imt);
        }
    } else {
        pDevObj->basicResults.getLoopCond.rloop = VP_INT16_MAX;
    }
    ...
    if (VP886_IS_TRACKER(pDevObj)) {
        /* For tracker, ch0 always uses SWY (bat1) and ch1 always uses SWZ (bat2) */
        if (pLineObj->channelId == 0) {
            pDevObj->basicResults.getLoopCond.selectedBat = VP_BATTERY_1;
        } else {
            pDevObj->basicResults.getLoopCond.selectedBat = VP_BATTERY_2;
        }
    } else { /* ABS */
        /* For ABS, read ICR1 to find which battery is in use */
        uint8 icr1[VP886_R_ICR1_LEN];
        VpSlacRegRead(NULL, pLineCtx, VP886_R_ICR1_RD, VP886_R_ICR1_LEN, icr1);
        if ((icr1[3] & VP886_R_ICR1_BAT) == VP886_R_ICR1_BAT_VBL) {
            pDevObj->basicResults.getLoopCond.selectedBat = VP_BATTERY_1;
        } else {
            pDevObj->basicResults.getLoopCond.selectedBat = VP_BATTERY_2;
        }
    ...
    convConf[0] = VP886_R_SADC_MATH | VP886_R_SADC_DRATE_SINGLE_2KHZ;
    ...
    /* Restore icr settings if cached in disconnect */
    if (pLineObj->getLoopCondRegsCached) {
    ...
    pLineObj->busyFlags &= ~VP886_LINE_GET_LOOP_COND;
    pLineObj->getLoopCondRegsCached = FALSE;
    ...
    uint16 handle)
{
    ...
    /* If initialization has not been started or if SPI/ZSI communication is
       blocked, we can't push the response event. */
    if (!(pDevObj->busyFlags & (VP_DEV_INIT_CMP | VP_DEV_INIT_IN_PROGRESS))) {
        Vp886ExitCritical(pDevCtx, NULL, "Vp886Query");
        return VP_STATUS_DEV_NOT_INITIALIZED;
    }
    if (pDevObj->options.accessCtrl == VP_ACCESS_CTRL_BLOCK_DEVICE) {
        Vp886ExitCritical(pDevCtx, NULL, "Vp886Query");
    ...
    switch (queryId) {
        ...
        case VP_QUERY_ID_LINE_TOPOLOGY:
            immediate = TRUE;
            break;
        case VP_QUERY_ID_SW_DUTY_CYCLE:
            status = Vp886QuerySwDutyCycleStart(pLineCtx, handle);
            immediate = FALSE;
            break;
        case VP_QUERY_ID_TEMPERATURE:
            status = Vp886QueryTemperatureStart(pLineCtx, handle);
            immediate = FALSE;
            break;
        default:
    ...
    if (immediate == TRUE && status == VP_STATUS_SUCCESS) {
        /* This only puts the event into the queue.  The actual result value
           is retrieved when VpGetResults() is called on the event. */
...
/** Vp886QuerySwDutyCycleStart()
  Performs the initial operations for VpQuery(VP_QUERY_ID_SW_DUTY_CYCLE).
  - Enable switcher on-time counting
  - Save current state info
  - Read the on-time register to reset it
  - Set a timer to run Vp886QuerySwDutyCycleFinish() after a delay
    - The 'handle' input to VpQuery() is passed through the timer
*/
VpStatusType
Vp886QuerySwDutyCycleStart(
    VpLineCtxType *pLineCtx,
    uint16 handle)
{
    ...
    uint8 channelId = pLineObj->channelId;
    uint8 swCtrl;
    uint8 swOnTime[VP886_R_SW_ON_TIME_LEN];

    if (pLineObj->swDutyCycle.inProgress) {
        VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886QuerySwDutyCycleStart - Already in progress"));
        return VP_STATUS_DEVICE_BUSY;
    }

    /* If switcher on-time counting is not enabled, enable it */
    if ((pLineObj->registers.swParam[0] & VP886_R_SWPARAM_COUNTER) == 0) {
        pLineObj->registers.swParam[0] |= VP886_R_SWPARAM_COUNTER;
        VpSlacRegWrite(pDevCtx, NULL, VP886_R_SWPARAM_WRT, VP886_R_SWPARAM_LEN, pLineObj->registers.swParam);
    }

    /* Save the current switcher mode */
    VpSlacRegRead(pDevCtx, NULL, VP886_R_SWCTRL_RD, VP886_R_SWCTRL_LEN, &swCtrl);
    if (channelId == 0) {
        pLineObj->swDutyCycle.switcherMode = (swCtrl & VP886_R_SWCTRL_MODE_Y);
    } else {
        pLineObj->swDutyCycle.switcherMode = ((swCtrl & VP886_R_SWCTRL_MODE_Z) >> 2);
    }

    /* Save the current line state and condition */
    pLineObj->swDutyCycle.lineState = pLineObj->lineState.currentState;
    pLineObj->swDutyCycle.lineCondition = pLineObj->lineState.condition;

    /* Read the switcher on-time register to reset the counts */
    VpSlacRegRead(pDevCtx, NULL, VP886_R_SW_ON_TIME_RD, VP886_R_SW_ON_TIME_LEN, swOnTime);

    /* Set a timer to read the on-time register again */
    if (!Vp886AddTimerMs(VP_NULL, pLineCtx, VP886_TIMERID_QUERY_SW_DUTY_CYCLE, VP886_QUERY_SW_DUTY_CYCLE_TIME, 0, handle)) {
        return VP_STATUS_DEVICE_BUSY;
    }

    pLineObj->swDutyCycle.inProgress = TRUE;
...
/** Vp886QuerySwDutyCycleStart()
  Performs the finishing operations for VpQuery(VP_QUERY_ID_SW_DUTY_CYCLE).
  - Read the switcher on-time register
  - Check if any important state changes have happened which may make the
    results unreliable.  If so, flag it in the results.
  - Calculate duty cycle percentage
  - Fill the VpQuery() result structure
  - Generate the QUERY_CMP event
*/
void
Vp886QuerySwDutyCycleFinish(
    VpLineCtxType *pLineCtx,
    uint16 handle)
{
    VpDevCtxType *pDevCtx = pLineCtx->pDevCtx;
    ...
    uint8 swFS;
    uint8 swOnTime[VP886_R_SW_ON_TIME_LEN];
    uint16 switcherPeriodReg;
    uint8 maxPulseWidthReg;
    uint16 onTime;
    uint32 onTimeClocks;
    uint16 cycleCount;
    uint32 cycleCountClocks;

    VpSlacRegRead(pDevCtx, NULL, VP886_R_SWCTRL_RD, VP886_R_SWCTRL_LEN, &swCtrl);
    if (channelId == 0) {
        swMode = (swCtrl & VP886_R_SWCTRL_MODE_Y);
    } else {
        swMode = ((swCtrl & VP886_R_SWCTRL_MODE_Z) >> 2);
    }
    swFS = swCtrl & VP886_R_SWCTRL_FREQ;

    /* Read the on-time and cycle count.  Fill the results directly from the
       register values */
    VpSlacRegRead(pDevCtx, NULL, VP886_R_SW_ON_TIME_RD, VP886_R_SW_ON_TIME_LEN, swOnTime);
    onTime = ((swOnTime[0] << 8) | swOnTime[1]);
    cycleCount = ((swOnTime[2] << 8) | swOnTime[3]);

    pLineObj->swDutyCycle.result.onTime = onTime;
    pLineObj->swDutyCycle.result.cycleCount = cycleCount;


    /* Use the on-time, cycle count, and switcher timing params to calculate
       the duty cycle % */
    switch(swMode) {
        case VP886_R_SWCTRL_MODE_LP:
            switcherPeriodReg = ((uint16)(pDevObj->registers.swTiming[0] & VP886_R_SWTIMING_PERIOD_MSB)) << 8;
            switcherPeriodReg |= (pDevObj->registers.swTiming[1] & VP886_R_SWTIMING_PERIOD_LSB);
            maxPulseWidthReg = (pDevObj->registers.swTiming[0] & VP886_R_SWTIMING_ONTIME) >> 2;
            pLineObj->swDutyCycle.result.powerMode = VP_SW_POWER_MODE_LOW;
            break;
        case VP886_R_SWCTRL_MODE_MP:
            switcherPeriodReg = ((uint16)(pDevObj->registers.swTiming[2] & VP886_R_SWTIMING_PERIOD_MSB)) << 8;
            switcherPeriodReg |= (pDevObj->registers.swTiming[3] & VP886_R_SWTIMING_PERIOD_LSB);
            maxPulseWidthReg = (pDevObj->registers.swTiming[2] & VP886_R_SWTIMING_ONTIME) >> 2;
            pLineObj->swDutyCycle.result.powerMode = VP_SW_POWER_MODE_MEDIUM;
            break;
        case VP886_R_SWCTRL_MODE_HP:
            switcherPeriodReg = ((uint16)(pDevObj->registers.swTiming[4] & VP886_R_SWTIMING_PERIOD_MSB)) << 8;
            switcherPeriodReg |= (pDevObj->registers.swTiming[5] & VP886_R_SWTIMING_PERIOD_LSB);
            maxPulseWidthReg = (pDevObj->registers.swTiming[4] & VP886_R_SWTIMING_ONTIME) >> 2;
            pLineObj->swDutyCycle.result.powerMode = VP_SW_POWER_MODE_HIGH;
            break;
        case VP886_R_SWCTRL_MODE_OFF:
            /* If switcher is off, return 0 for all calculated results */
            pLineObj->swDutyCycle.result.switcherPeriod = 0;
            pLineObj->swDutyCycle.result.maxPulseWidth = 0;
            pLineObj->swDutyCycle.result.avgDutyCycle = 0;
            pLineObj->swDutyCycle.result.powerMode = VP_SW_POWER_MODE_OFF;
            return;
        default:
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886QuerySwDutyCycleFinish - bad swMode %02X.  swCtrl %02X\n", swMode, swCtrl));
            return;
    }

    /* Convert on-time from 6.144MHz clock ticks to 24.576MHz ticks */
    onTimeClocks = (uint32)onTime * 4;

    /* Use cycleCount and switching period to get the total ticks.  This will
       need to be adjusted to 24.576MHz ticks based on swFS */
    cycleCountClocks = (uint32)cycleCount * switcherPeriodReg;

    /* Use swFS to adjust cycleCountClocks, switcher period, and max pulse width
       all to 24.576Mhz clock ticks */
    if (swFS == VP886_R_SWCTRL_FREQ_24MHZ) {
        cycleCountClocks *= 1;
        pLineObj->swDutyCycle.result.switcherPeriod = switcherPeriodReg;
        pLineObj->swDutyCycle.result.maxPulseWidth = maxPulseWidthReg;
    } else if (swFS == VP886_R_SWCTRL_FREQ_12MHZ) {
        cycleCountClocks *= 2;
        pLineObj->swDutyCycle.result.switcherPeriod = switcherPeriodReg * 2;
        pLineObj->swDutyCycle.result.maxPulseWidth = maxPulseWidthReg * 2;
    } else if (swFS == VP886_R_SWCTRL_FREQ_6MHZ) {
        cycleCountClocks *= 4;
        pLineObj->swDutyCycle.result.switcherPeriod = switcherPeriodReg * 4;
        pLineObj->swDutyCycle.result.maxPulseWidth = maxPulseWidthReg * 4;
    }

    /* Result is in units of hundredths of percent */
    if (cycleCountClocks == 0) {
        VP_WARNING(VpLineCtxType, pLineCtx, ("Vp886QuerySwDutyCycleFinish - cycleCountClocks is 0.  cycleCount=%d, periodReg=0x%04X, swFS=0x%02X",
            cycleCount, switcherPeriodReg, swFS));
        pLineObj->swDutyCycle.result.avgDutyCycle = 0;
    } else if (onTimeClocks > cycleCountClocks) {
        VP_WARNING(VpLineCtxType, pLineCtx, ("Vp886QuerySwDutyCycleFinish - unexpected (onTimeClocks (%d) > cycleCountClocks (%d))",
            onTimeClocks, cycleCountClocks));
        pLineObj->swDutyCycle.result.avgDutyCycle = 100 * 100;
    } else {
        pLineObj->swDutyCycle.result.avgDutyCycle = (uint16)VP_ROUNDED_DIVIDE(onTimeClocks * 100 * 100, cycleCountClocks);
    }


    /* If the switcher mode, line state, or line condition (hook, gkey, etc) has
       changed during the measurement time, set a flag to indicate that there
       was a change of state that makes the result unreliable. */
    pLineObj->swDutyCycle.result.stateChanged = FALSE;

    if (swMode != pLineObj->swDutyCycle.switcherMode) {
        VP_INFO(VpLineCtxType, pLineCtx, ("SW_DUTY_CYCLE - Switcher mode changed (%02X -> %02X)",
            pLineObj->swDutyCycle.switcherMode, swMode));
        pLineObj->swDutyCycle.result.stateChanged = TRUE;
    }
    if (pLineObj->lineState.currentState != pLineObj->swDutyCycle.lineState) {
        VP_INFO(VpLineCtxType, pLineCtx, ("SW_DUTY_CYCLE - Line state changed (%d -> %d)",
            pLineObj->swDutyCycle.lineState, pLineObj->lineState.currentState));
        pLineObj->swDutyCycle.result.stateChanged = TRUE;
    }
    if (pLineObj->lineState.condition != pLineObj->swDutyCycle.lineCondition) {
        VP_INFO(VpLineCtxType, pLineCtx, ("SW_DUTY_CYCLE - Line condition changed (%04X -> %04X)",
            pLineObj->swDutyCycle.lineCondition, pLineObj->lineState.condition));
        pLineObj->swDutyCycle.result.stateChanged = TRUE;
    }

    Vp886PushEvent(pDevCtx, channelId, VP_EVCAT_RESPONSE, VP_LINE_EVID_QUERY_CMP, VP_QUERY_ID_SW_DUTY_CYCLE, handle, TRUE);

    return;
}

/** Vp886QueryTemperatureStart()
  Performs the initial operations for VpQuery(VP_QUERY_ID_TEMPERATURE).
  - Check for conflicting operations
  - Start the SADC temperature measurement
*/
VpStatusType
Vp886QueryTemperatureStart(
    VpLineCtxType *pLineCtx,
    uint16 handle)
{
...
#ifdef VP886_INCLUDE_ADAPTIVE_RINGING
    {
        ...
        if ((pDevObj->options.adaptiveRinging.power != VP_ADAPTIVE_RINGING_DISABLED) && 
            ((pLineObj->lineState.usrCurrent == VP_LINE_RINGING) ||
            (pLineObj->lineState.usrCurrent == VP_LINE_RINGING_POLREV))) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("VP_QUERY_ID_TEMPERATURE - Cannot run during adaptive ringing"));
            return VP_STATUS_DEVICE_BUSY;
        }
    }
#endif
...
#ifdef VP886_INCLUDE_TESTLINE_CODE
    if (pLineObj->inLineTest) {
        VP_ERROR(VpLineCtxType, pLineCtx, ("VP_QUERY_ID_TEMPERATURE - Cannot run during line test"));
        return VP_STATUS_DEVICE_BUSY;
    }
#endif

    if (pLineObj->busyFlags & VP886_LINE_GET_LOOP_COND) {
        VP_ERROR(VpLineCtxType, pLineCtx, ("VP_QUERY_ID_TEMPERATURE - Cannot run during VpGetLoopCond()"));
        return VP_STATUS_DEVICE_BUSY;
    }

    pLineObj->busyFlags |= VP886_LINE_QUERY_TEMP;

#ifdef VP886_INCLUDE_DTMF_DETECT
    /* Shut down DTMF detection if it is running.  This must come after setting
       the VP886_LINE_QUERY_TEMP busy-flag. */
    Vp886DtmfManage(pLineCtx);
#endif
    ...
    /* Program the SADC routes */
    convConf[1] = VP886_R_SADC_SEL_TEMP;
    convConf[2] = VP886_R_SADC_SEL_ADC_OFFSET;
    convConf[3] = VP886_R_SADC_SEL_ADC_OFFSET;
    convConf[4] = VP886_R_SADC_SEL_ADC_OFFSET;
    convConf[5] = VP886_R_SADC_SEL_ADC_OFFSET;

    /* Skip 5 and collect 1 sample.  The temperature measurement needs just a
       little bit of settling time. */
    convConf[6] = 0x00;
    convConf[7] = 1;
    convConf[8] = 0x00;
    convConf[9] = 5;
    ...
    pLineObj->queryTempHandle = handle;

    return VP_STATUS_SUCCESS;
}


/** Vp886QueryTemperatureFinish()
  Performs the finishing operations for VpQuery(VP_QUERY_ID_TEMPERATURE).
  - Read the SADC measurement result
  - Disable SADC
  - Clear flags
  - Generate QUERY_CMP event
*/
void
Vp886QueryTemperatureFinish(
    VpLineCtxType *pLineCtx)
{
    ...
    pLineObj->queryTempResult = ((int16)dataBuff[0]) << 8 | dataBuff[1];
    ...
    Vp886PushEvent(pDevCtx, channelId, VP_EVCAT_RESPONSE, VP_LINE_EVID_QUERY_CMP, VP_QUERY_ID_TEMPERATURE, pLineObj->queryTempHandle, TRUE);
    ...
    void *pResults)
{
    ...
    switch (queryId) {
        ...
        case VP_QUERY_ID_LINE_TOPOLOGY:
            break;
        case VP_QUERY_ID_SW_DUTY_CYCLE:
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886QueryImmediate - SW_DUTY_CYCLE only valid through VpQuery()"));
            Vp886ExitCritical(VP_NULL, pLineCtx, "Vp886QueryImmediate");
            return VP_STATUS_INVALID_ARG;
        case VP_QUERY_ID_TEMPERATURE:
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886QueryImmediate - TEMPERATURE only valid through VpQuery()"));
            Vp886ExitCritical(VP_NULL, pLineCtx, "Vp886QueryImmediate");
            return VP_STATUS_INVALID_ARG;
        default:
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886QueryImmediate - Invalid queryId %d", queryId));
            Vp886ExitCritical(VP_NULL, pLineCtx, "Vp886QueryImmediate");
            return VP_STATUS_INVALID_ARG;
    }
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    if (pDevObj->options.accessCtrl == VP_ACCESS_CTRL_BLOCK_DEVICE) {
        Vp886ExitCritical(pDevCtx, NULL, "Vp886RegisterDump");
        return VP_STATUS_ACCESS_BLOCKED;
    }
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    if (pDevCtx != VP_NULL) {
        ...
        VpSysDebugPrintf("\n\rpDevObj->trafficBytes = %u", pDevObj->trafficBytes);
        ...
        VpSysDebugPrintf("\n\rpDevObj->rolloverBuffer = %d", pDevObj->rolloverBuffer);
        ...
        if (pDevObj->swTimingParamsBattBackup[0] != 0 || pDevObj->swTimingParamsBattBackup[1] != 0 || pDevObj->swTimingParamsBattBackup[2] != 0 || 
            pDevObj->swTimingParamsBattBackup[3] != 0 || pDevObj->swTimingParamsBattBackup[4] != 0 || pDevObj->swTimingParamsBattBackup[5] != 0) {
            uint8 byteCount;
            VpSysDebugPrintf("\n\rpDevObj->swTimingParamsBattBackup =");
            for (byteCount = 0; byteCount < VP886_R_SWTIMING_LEN; byteCount++) {
                VpSysDebugPrintf(" 0x%02X", pDevObj->swTimingParamsBattBackup[byteCount]);
            }
        }
        ...
        VpSysDebugPrintf("\n\rpDevObj->softStartDuration = %d", pDevObj->softStartDuration);
        VpSysDebugPrintf("\n\rpDevObj->swTimingParamsSoftStart = 0x%02X 0x%02X",
            pDevObj->swTimingParamsSoftStart[0], pDevObj->swTimingParamsSoftStart[1]);
        ...
        if (VP886_IS_ABS(pDevObj)) {
            ...
            VpSysDebugPrintf("\n\rpDevObj->batSelGpioState = %d", pDevObj->batSelGpioState);

            VpSysDebugPrintf("\n\n\rpDevObj->battBackup.vsw = %d", pDevObj->battBackup.vsw);
            VpSysDebugPrintf("\n\rpDevObj->battBackup.swy = %d", pDevObj->battBackup.swy);
            VpSysDebugPrintf("\n\rpDevObj->battBackup.vbhOverhead = %d", pDevObj->battBackup.vbhOverhead);
        ...
        VpSysDebugPrintf("\n\rpDevObj->overCurrentTimestamp[0] = %u", pDevObj->overCurrentTimestamp[0]);
        VpSysDebugPrintf("\n\rpDevObj->overCurrentTimestamp[1] = %u", pDevObj->overCurrentTimestamp[1]);

        VpSysDebugPrintf("\n\n\rpDevObj->spiError.count = %u", pDevObj->spiError.count);
        VpSysDebugPrintf("\n\rpDevObj->spiError.block = %s", pDevObj->spiError.block ? "TRUE" : "FALSE");
        VpSysDebugPrintf("\n\rpDevObj->spiError.clearMpi = %s", pDevObj->spiError.clearMpi ? "TRUE" : "FALSE");
        VpSysDebugPrintf("\n\rpDevObj->spiError.skipGetEvent = %s", pDevObj->spiError.skipGetEvent ? "TRUE" : "FALSE");
        ...
        VpSysDebugPrintf("\n\rpDevObj->registers.swTiming = 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            pDevObj->registers.swTiming[0], pDevObj->registers.swTiming[1],
            pDevObj->registers.swTiming[2], pDevObj->registers.swTiming[3],
            pDevObj->registers.swTiming[4], pDevObj->registers.swTiming[5]);
        ...
        VpSysDebugPrintf("\n\rpDevObj->registers.fuse8 = 0x%02X 0x%02X 0x%02X 0x%02X",
            pDevObj->registers.fuse8[0], pDevObj->registers.fuse8[1],
            pDevObj->registers.fuse8[2], pDevObj->registers.fuse8[3]);

        VpSysDebugPrintf("\n\n\rpDevObj->debugSelectMask = 0x%08X", pDevObj->debugSelectMask);
    ...
    if (pLineCtx != VP_NULL) {
        ...
        if (pLineObj->dtmf.sampling) {
            ...
            VpSysDebugPrintf("\n\rpLineObj->dtmf.detectData.totalEnergy = %u", pLineObj->dtmf.detectData.totalEnergy);
        }
#endif
        ...
        if (VP886_IS_ABS(pDevObj)) {
            VpSysDebugPrintf("\n\n\rpLineObj->midBatCompEnable = %s", pLineObj->midBatCompEnable ? "TRUE" : "FALSE");
            VpSysDebugPrintf("\n\rpLineObj->midBatCompValue = 0x%08X", pLineObj->midBatCompValue);
        }
       ...
       VpSysDebugPrintf("\n\n\rpLineObj->debugSelectMask = 0x%08X ", pLineObj->debugSelectMask);
       
       VpSysDebugPrintf("\n\n\rpLineObj->refRingingBatV = %i ", pLineObj->refRingingBatV);
