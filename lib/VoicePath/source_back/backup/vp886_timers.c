    ...
    uint32 handle)
{
    ...
    VP_TIMER(VpDevCtxType, pDevCtx, ("Vp886AddTimerHalfMs id %d, dur %u, ovr %u, ch %d, hndl %u", timerId, duration, overrun, channelId, handle));
    ...
    if (!success) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886AddTimerHalfMs - Failed to insert timer id %u, ch %d, hndl %u", timerId, channelId, handle));
    }
    ...
    uint32 handle)
{
    ...
    VP_TIMER(VpDevCtxType, pDevCtx, ("Vp886RestartTimerHalfMs id %d, dur %u, ch %d, hndl %u", timerId, duration, channelId, handle));

    success = VpRestartTimer(pDevCtx, pInfo, pNodes, timerId, duration, channelId, handle);
    if (!success) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886RestartTimerHalfMs - Failed to restart timer id %u, ch %d, hdnl %u", timerId, channelId, handle));
    }
    ...
    uint32 handle)
{
    ...
    VP_TIMER(VpDevCtxType, pDevCtx, ("Vp886ExtendTimerMs id %d, ms %u, ch %d, hndl %u", timerId, duration, channelId, handle));

    success = VpExtendTimer(pDevCtx, pInfo, pNodes, timerId, durationHalfMs, channelId, handle);
    if (!success) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886ExtendTimerMs - Failed to extend timer id %u, ch %d, hdnl %u", timerId, channelId, handle));
    }
    ...
    bool matchHandle)
{
    ...
    if (!success) {
        VP_TIMER(VpDevCtxType, pDevCtx, ("Vp886CancelTimer - Timer id %u, ch %d, hndl %u did not exist to be removed", timerId, channelId, handle));
    }
    return success;
}
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    if (pDevObj->rolloverBuffer > 0) {
        pDevObj->rolloverCount += (uint16)pDevObj->rolloverBuffer;
    ...
    oldTimestamp = pDevObj->timestamp;
    ...
    if (timestamp == 0x0000 || timestamp == 0xFFFF) {
            ...
            ((rcnPcn[0] == 0x00) && (rcnPcn[1] == 0x00)))
        {
            ...
            /* Return the old timestamp, rather than the bad one */
            pDevObj->timestamp = oldTimestamp;
            return oldTimestamp;
        }
    }
    ...
    uint32 duration)
{
    ...
    VP_TIMER(VpDevCtxType, pDevCtx, ("Vp886SetDeviceTimer %u, %s", duration, enable ? "ON" : "OFF"));
    ...
    if (duration > VP886_R_GTIMER_TIME_MAXIMUM) {
        ...
        VP_TIMER(VpDevCtxType, pDevCtx, ("Vp886SetDeviceTimer setting maximum, %u", duration));
    }
    ...
    uint32 handle)
{
    ...
    VP_TIMER(VpDevCtxType, pDevCtx, ("Vp886TimerHandler id %d, or %u, ch %d, arg %u, ts %u", timerId, overrun, channelId, handle, timestamp));
    ...
    switch (timerId) {
        ...
        case VP886_TIMERID_RING_EXIT_CLEANUP: {
            ...
            switch (handle) {
                ...
                default:
                    VP_WARNING(VpLineCtxType, pLineCtx, ("Ring exit state cleanup, invalid state %u", handle));
            }
            break;
        }
        ...
        case VP886_TIMERID_USER: {
            ...
            Vp886PushEvent(pDevCtx, channelId, VP_EVCAT_PROCESS, VP_LINE_EVID_GEN_TIMER, VP_GEN_TIMER_STATUS_CMP, (uint16)handle, FALSE);
            break;
        }
        ...
        case VP886_TIMERID_SWSKIP_CHECK: {
            /* Unmask SW_SKIP to check whether it is still happening */
            ...
            /* Set a timer to clear the status flag if the interrupt does not
               occur */
            Vp886RestartTimerMs(pDevCtx, VP_NULL, VP886_TIMERID_SWSKIP_CLEAR, VP886_SWSKIP_CLEAR_TIME, 0);
            break;
        }
        case VP886_TIMERID_SWSKIP_CLEAR: {
            /* No SW_SKIP interrupts detected, clear the flag */
            pDevObj->swSkipPresent = FALSE;
            VP_WARNING(VpDevCtxType, pDevCtx, ("SW_SKIP cleared, ts %d", Vp886GetTimestamp(pDevCtx)));
            break;
        }
        case VP886_TIMERID_QUERY_SW_DUTY_CYCLE: {
            if (pLineCtx == VP_NULL) {
                VP_WARNING(VpDevCtxType, pDevCtx, ("VP886_TIMER_ID_QUERY_SW_DUTY_CYCLE expired on ch%d, but no line context available",
                    channelId));
                break;
            }
            Vp886QuerySwDutyCycleFinish(pLineCtx, (uint16)handle);
            break;
        }
        ...
        case VP886_TIMERID_THERMAL_RINGING: {
            ...
            switch (handle) {
                ...
                default:
                    VP_WARNING(VpLineCtxType, pLineCtx, ("Adaptive ringing, invalid state %u", handle));
            }
            break;
        }
#endif        
