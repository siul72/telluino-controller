    ...
    VpLineCtxType *pLineCtx)
{
    ...
    if (pLineCtx == VP_NULL) {
        ...
        case VP_DEV_MELT792_SERIES:
            retval = Melt792ReadCalFlag(pLineCtx);
            break;
#endif
...
#ifdef VP_CC_MELT792_SERIES
        case VP_DEV_MELT792_SERIES:
            Melt792SetCalFlag(pLineCtx, value);
            break;
#endif
...
/**
 * VpSealCurCtrl()
 *  This function sets the sealing current control for a MeLT792 test resource.
 *
 * Preconditions:
 *  Line context should be created and initialized. For applicable
 * devices bootload should be performed before calling the function.
 *
 * Postconditions:
 *  The sealing current state state will change as designated by ctrl.
 */
VpStatusType
VpSealCurCtrl(
    VpDevCtxType *pDevCtx, 
...
{
    VpStatusType status;
    VP_API_ENTER(VpDevCtxType, pDevCtx, "SealCurCtrl");

    /* Basic argument checking */
    if (pDevCtx == VP_NULL) {
        status = VP_STATUS_INVALID_ARG;
    } else {
        status = VP_CALL_DEV_FUNC(SealCurCtrl, (pDevCtx, ctrl));
    }

    VP_API_EXIT(VpDevCtxType, pDevCtx, "SealCurCtrl", status);
    return status;
} /* VpSealCurCtrl() */
...
/*
 * VpBatteryBackupMode()
 *  This function modifies the switching regulator timing parameters to
 *  operate in battery backup mode or to return to normal mode.
 *
 * Preconditions:
 *  The device profile used previously in VpInitDevice() must contain valid
 *  battery backup timing parameters.  For BBABS, the new switcher input voltage
 *  (vsw) should be provided in 0.2V steps.
 *
 * Postconditions:
 *  Switching regulator timing parameters will be adjusted as necessary for
 *  the requested mode.
 */
VpStatusType
VpBatteryBackupMode(
...
{
    VpStatusType status;
    VP_API_ENTER(VpDevCtxType, pDevCtx, "BatteryBackupMode");

    /* Basic argument checking */
    if (pDevCtx == VP_NULL) {
        status = VP_STATUS_INVALID_ARG;
    } else {
        status = VP_CALL_DEV_FUNC(BatteryBackupMode, (pDevCtx, backupMode, vsw));
    }

    VP_API_EXIT(VpDevCtxType, pDevCtx, "BatteryBackupMode", status);
    return status;
} /* VpBatteryBackupMode() */
