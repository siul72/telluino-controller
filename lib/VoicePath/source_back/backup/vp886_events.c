    ...
    VpEventType *pEvent)    /**< Pointer to the results event structure */
{
    ...
    if (pDevObj->options.accessCtrl == VP_ACCESS_CTRL_BLOCK_DEVICE) {
        pEvent->status = VP_STATUS_ACCESS_BLOCKED;
        Vp886ExitCritical(pDevCtx, NULL, "Vp886GetEvent");
        return FALSE;
    }
    ...
    /* Don't read the signaling register if there is a SPI error block */
    if (pDevObj->spiError.block) {
    ...
    /* When VpGetEvent() returns a SPI error event, we want to break out of
       the application-side "while(VpGetEvent==TRUE)" loop. */
    if (pDevObj->spiError.skipGetEvent) {
    ...
    while (pDevObj->sigregReadPending && numReads < maxReads) {
        ...
        /* Check the signaling register for impossible bits that would indicate
           a SPI error. */
        if (Vp886SigRegSpiCheck(pDevCtx, sigreg) == FALSE) {
            Vp886ReportSpiError(pDevCtx);
            break;
        }
...
/** Vp886SigRegSpiCheck()
  This function checks the signaling register contents to see if any of the
  reserved bits are set.  If so, that indicates a SPI error.  Previously we only
  looked for 0xFFFFFFFF, but not all failures look like that.  ZSI failures
  for instance will sometimes just have one of the bytes set to 0xF1, and this
  test is more likely to catch those cases.
*/
bool
Vp886SigRegSpiCheck(
    VpDevCtxType *pDevCtx,
...
{
    Vp886DeviceObjectType *pDevObj = pDevCtx->pDevObj;
    uint8 rsvdMask[VP886_R_SIGREG_LEN];
    
    /* Set up the reserved bit masks.  Different device versions have different
       reserved bits */
    if (VP886_IS_SF1(pDevObj)) {
        rsvdMask[0] = VP886_R_SIGREG_SF1_BYTE0_RSVD;
        rsvdMask[1] = VP886_R_SIGREG_SF1_BYTE1_RSVD;
        rsvdMask[2] = VP886_R_SIGREG_SF1_BYTE2_RSVD;
        rsvdMask[3] = VP886_R_SIGREG_SF1_BYTE3_RSVD;
    } else if (VP886_IS_SF2(pDevObj)) {
        rsvdMask[0] = VP886_R_SIGREG_SF2_BYTE0_RSVD;
        rsvdMask[1] = VP886_R_SIGREG_SF2_BYTE1_RSVD;
        rsvdMask[2] = VP886_R_SIGREG_SF2_BYTE2_RSVD;
        rsvdMask[3] = VP886_R_SIGREG_SF2_BYTE3_RSVD;
    } else {
        rsvdMask[0] = VP886_R_SIGREG_A2_BYTE0_RSVD;
        rsvdMask[1] = VP886_R_SIGREG_A2_BYTE1_RSVD;
        rsvdMask[2] = VP886_R_SIGREG_A2_BYTE2_RSVD;
        rsvdMask[3] = VP886_R_SIGREG_A2_BYTE3_RSVD;
    }

    /* None of the bits of the reserved mask should be present in the sigreg */
    if ((sigreg[0] & rsvdMask[0]) != 0x00 ||
        (sigreg[1] & rsvdMask[1]) != 0x00 ||
        (sigreg[2] & rsvdMask[2]) != 0x00 ||
        (sigreg[3] & rsvdMask[3]) != 0x00)
    {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886SigRegSpiCheck() - SPI error in sigreg 0x%02X%02X%02X%02X",
            sigreg[0], sigreg[1], sigreg[2], sigreg[3]));
        return FALSE;
    }

    /* If the current sigreg is all 0, and the previous valid sigreg was not,
       do a sanity check of the RCN/PCN */
    if ( (sigreg[0] == 0x00 &&
          sigreg[1] == 0x00 &&
          sigreg[2] == 0x00 &&
          sigreg[3] == 0x00) &&
        !(pDevObj->registers.sigreg[0] == 0x00 &&
          pDevObj->registers.sigreg[1] == 0x00 &&
          pDevObj->registers.sigreg[2] == 0x00 &&
          pDevObj->registers.sigreg[3] == 0x00) )
    {
        uint8 rcnPcn[VP886_R_RCNPCN_LEN];
        VpSlacRegRead(pDevCtx, NULL, VP886_R_RCNPCN_RD, VP886_R_RCNPCN_LEN, rcnPcn);
        if (rcnPcn[0] == 0x00 && rcnPcn[1] == 0x00) {
            VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886SigRegSpiCheck() - SPI error, sigreg 0x%02X%02X%02X%02X, RCN/PCN 0x%02X%02X",
                sigreg[0], sigreg[1], sigreg[2], sigreg[3], rcnPcn[0], rcnPcn[1]));
            return FALSE;
        }
    }
    ...
    uint8 sigreg[])
{
    if ((sigreg[0] & (VP886_R_SIGREG_CAD | VP886_R_SIGREG_CID |
        ...
        (sigreg[3] & (VP886_R_SIGREG_TIMESTAMP | VP886_R_SIGREG_CHTIMER |
            VP886_R_SIGREG_SDAT | VP886_R_SIGREG_VDAT | VP886_R_SIGREG_SWSKIP)))
    {
        return TRUE;
    } else {
        return FALSE;
    ...
    uint8 sigreg[])
{
    ...
    if (sigreg[3] & VP886_R_SIGREG_CHTIMER) {
        ...
        for (channelId = 0; channelId < pDevObj->staticInfo.maxChannels; channelId++) {
            ...
            (void)pLineObj; /* prevent compile time warning when the following #define is false */
...
#endif /* defined(VP886_INCLUDE_DTMF_DETECT) && !defined(VP886_DTMF_DETECT_NONCONTINUOUS) */
        }
    }
    /* - CH2 Timer */

    /* + CH1 Timer */
    /* Used for immediate interrupts */
    if (sigreg[2] & VP886_R_SIGREG_CHTIMER) {
        uint8 channelId;
        for (channelId = 0; channelId < pDevObj->staticInfo.maxChannels; channelId++) {
            pLineCtx = pDevCtx->pLineCtx[channelId];
            if (pLineCtx == VP_NULL) {
            ...
            (void)pLineObj; /* prevent compile time warning when the following #define is false */
#if defined(VP886_INCLUDE_DTMF_DETECT) && !defined(VP886_DTMF_DETECT_NONCONTINUOUS)
            /* When first starting up the DTMF sampling timer, we must first
               utilize an immediate interrupt in order to avoid an overflow
               corner case.  See zforge #11997.
               This is deliberately done AFTER the CH2 timer handling to avoid
               calling the sampling function twice. */
            if (pLineObj->dtmf.sampling && pLineObj->dtmf.initialSample) {
                Vp886DtmfSampling(pLineCtx);
            }
#endif /* defined(VP886_INCLUDE_DTMF_DETECT) && !defined(VP886_DTMF_DETECT_NONCONTINUOUS) */
        }
    ...
    /* + Switcher Skip */
    if (sigreg[3] & VP886_R_SIGREG_SWSKIP) {
        if (!pDevObj->swSkipPresent) {
            VP_WARNING(VpDevCtxType, pDevCtx, ("SW_SKIP detected, ts %d", Vp886GetTimestamp(pDevCtx)));
            pDevObj->swSkipPresent = TRUE;
        }
        /* Mask the SW_SKIP so that the application is not continually hammered
           with interrupts.  After some delay, we will unmask it again to check
           whether the condition remains or has ended. */
        pDevObj->registers.intMask[3] |= VP886_R_INTMASK_SWSKIP;
        VpSlacRegWrite(pDevCtx, VP_NULL, VP886_R_INTMASK_WRT, VP886_R_INTMASK_LEN, pDevObj->registers.intMask);
        Vp886CancelTimer(pDevCtx, VP_NULL, VP886_TIMERID_SWSKIP_CLEAR, 0, FALSE);
        Vp886RestartTimerMs(pDevCtx, VP_NULL, VP886_TIMERID_SWSKIP_CHECK, VP886_SWSKIP_CHECK_TIME, 0);
    }
    /* - Switcher Skip */
    ...
    uint8 sigreg[])
{
    ...
    if (sigreg[0] & VP886_R_SIGREG_OCALM) {
        ...
        VP_INFO(VpDevCtxType, pDevCtx, ("Cleared OC counter for ch0, ts %d",
    ...
    if (sigreg[1] & VP886_R_SIGREG_OCALM) {
        ...
        VP_INFO(VpDevCtxType, pDevCtx, ("Cleared OC counter for ch1, ts %d",
    ...
    if (newBattFltStatus != pDevObj->battFltStatus) {
        ...
        /* Update dynamicInfo.batXFault flags for compatibility with
           VpGetLineStatus() and VpGetDeviceStatus(). */
        if ((pDevObj->battFltStatus & VP_BAT_FLT_SWY_OV) ||
            (pDevObj->battFltStatus & VP_BAT_FLT_SWY_OC)) {
            pDevObj->dynamicInfo.bat1Fault = TRUE;
        } else {
            pDevObj->dynamicInfo.bat1Fault = FALSE;
        }
        if ((pDevObj->battFltStatus & VP_BAT_FLT_SWZ_OV) ||
            (pDevObj->battFltStatus & VP_BAT_FLT_SWZ_OC)) {
            pDevObj->dynamicInfo.bat2Fault = TRUE;
        } else {
            pDevObj->dynamicInfo.bat2Fault = FALSE;
        }
        if (pDevObj->battFltStatus & VP_BAT_FLT_CP_UV) {
    ...
    uint8 sigreg[])
{
    ...
    if (chanSignals[1] & VP886_R_SIGREG_SDAT) {
        ...
        /* SADC interrupt used for VP_QUERY_ID_TEMPERATURE */
        if (pLineObj->busyFlags & VP886_LINE_QUERY_TEMP) {
            Vp886QueryTemperatureFinish(pLineCtx);
        }
    ...
    uint16 timestamp)
{
    ...
    /* Take the dial pulse correction from the device profile (specified in ms)
       and subtract it from all on-hook timestamps. */
    if (newHookSt == VP_CSLAC_CLEAR) {
    ...
    uint16 timestamp)
{
        ...
        Vp886LineStateInfo(pLineObj->lineState.currentState).normalEquiv == VP_LINE_RINGING)
    {
        ...
        overrun = (uint16)VpRoundedDivide(Vp886GetTimestamp(pDevCtx) - timestamp, 2);
        ...
        if (ringFreq_mHz == 0) {
            ...
            ringTripDelay = (uint16)VpRoundedDivide(1000000, ringFreq_mHz) * pLineObj->ringTripCycles;
        }
    ...
    switch (option) {
        ...
        case VP_DEVICE_OPTION_ID_FSYNC_RATE:
            *(VpOptionFsyncRateType *)pResults = pOptions->fsyncRate;
            break;
        case VP_DEVICE_OPTION_ID_ACCESS_CTRL:
            *(VpOptionAccessCtrlType *)pResults = pOptions->accessCtrl;
            break;
        case VP_DEVICE_OPTION_ID_SPI_ERROR_CTRL:
            *(VpOptionSpiErrorCtrlType *)pResults = pOptions->spiErrorCtrl;
            break;
        default:
    ...
    VpQueryResultsType *pResults)
{
    ...
    switch (queryId) {
        ...
        case VP_QUERY_ID_SW_DUTY_CYCLE:
            pResults->swDutyCycle = pLineObj->swDutyCycle.result;
            pLineObj->swDutyCycle.inProgress = FALSE;
            break;
        case VP_QUERY_ID_TEMPERATURE:
            pResults->temp = pLineObj->queryTempResult;
            break;
        default:
    ...
    VpEventType *pEvent)
{
    ...
    if (pEvent->eventCategory == VP_EVCAT_FAULT &&
        ...
        pEvent->eventData == VP_SYS_FLT_SPI_ERROR)
    {
        /* If we're returning a SPI error event, schedule a NO-OP flush of the MPI
           bus to occur before the next attempted transaction.  Doing this here
           because there should be no more MPI commands before VpGetEvent() returns,
           so the NO-OP flush will happen with the next function call, potentially
           giving a temporary error condition some time to correct itself. */
        pDevObj->spiError.clearMpi = TRUE;
    }
...
/** Vp886ReportSpiError()
  Call when a SPI error is detected to report the event and manage relevant
  counts and flags.
*/
void
Vp886ReportSpiError(
    VpDevCtxType *pDevCtx)
{
    ...
    /* Set the timerOverride flag, which forces us to service timers
       with every VpGetEvent() call, regardless of whether a timer
       interrupt is seen.  This should make the system less likely to
       get stuck due to a missed timer interrupt when a SPI error occurs */
    pDevObj->timerOverride = TRUE;
    /* Skip the next VpGetEvent that would read the sigreg to prevent the
       application-side "while(VpGetEvent==TRUE)" loop from returning a
       flood of SPI error events */
    pDevObj->spiError.skipGetEvent = TRUE;

    /* Increment the error count, and set the block flag if we reach the
       threshold. */
    if (pDevObj->options.spiErrorCtrl.count != 0 &&
        pDevObj->spiError.count < pDevObj->options.spiErrorCtrl.count)
    {
        pDevObj->spiError.count++;
        if (pDevObj->spiError.count == pDevObj->options.spiErrorCtrl.count) {
            pDevObj->spiError.block = TRUE;
        }
    }

    Vp886PushEvent(pDevCtx, 0, VP_EVCAT_FAULT, VP_DEV_EVID_SYSTEM_FLT, VP_SYS_FLT_SPI_ERROR, 0, FALSE);
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    if (enable && !pLineObj->dtmf.sampling) {
        ...
        pLineObj->dtmf.initialSample = TRUE;
        VpDtmfDetectReset(pLineCtx, &pLineObj->dtmf.detectData, TRUE);
            ...
            /* When first starting up the DTMF sampling timer, we must first
               utilize an immediate interrupt in order to avoid an overflow
               corner case.  See zforge #11997. */
            if (pDevObj->samplingTimerCurrent == 0) {
                Vp886ForceTimerInterrupt(pDevCtx);
            }
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    if (newDigit != pLineObj->dtmf.currentDigit) {
        ...
        if (newDigit != VP_DIG_NONE) {
            ...
            if (dBm >= 100000) {
                ...
                output_dBm = (uint8)VpRoundedDivide(dBm + 537500, 2500);
            }

            VP_DTMF(VpLineCtxType, pLineCtx, ("Digit level: row %u, col %u, avg %u, sqrt %u, dBm %d, output %d",
    ...
    pLineObj->dtmf.initialSample = FALSE;
